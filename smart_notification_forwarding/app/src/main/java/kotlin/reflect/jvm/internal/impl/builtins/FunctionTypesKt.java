...
public final class FunctionTypesKt {
    public static final FunctionTypeKind getFunctionTypeKind(KotlinType kotlinType) {
        ...
        if (mo1386getDeclarationDescriptor != null) {
            return getFunctionTypeKind(mo1386getDeclarationDescriptor);
        }
        return null;
    }
    ...
    public static final boolean isBuiltinFunctionalType(KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        ClassifierDescriptor mo1386getDeclarationDescriptor = kotlinType.getConstructor().mo1386getDeclarationDescriptor();
        return mo1386getDeclarationDescriptor != null && isBuiltinFunctionalClassDescriptor(mo1386getDeclarationDescriptor);
    }
    ...
    private static final boolean isTypeAnnotatedWithExtensionFunctionType(KotlinType kotlinType) {
        return kotlinType.getAnnotations().mo1381findAnnotation(StandardNames.FqNames.extensionFunctionType) != null;
    }
    ...
    public static final int contextFunctionTypeParamsCount(KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        AnnotationDescriptor mo1381findAnnotation = kotlinType.getAnnotations().mo1381findAnnotation(StandardNames.FqNames.contextFunctionTypeParams);
        if (mo1381findAnnotation == null) {
            return 0;
        }
        ConstantValue constantValue = (ConstantValue) MapsKt.getValue(mo1381findAnnotation.getAllValueArguments(), StandardNames.CONTEXT_FUNCTION_TYPE_PARAMETER_COUNT_NAME);
    ...
    public static final Name extractParameterNameFromFunctionTypeArgument(KotlinType kotlinType) {
        String value;
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        AnnotationDescriptor mo1381findAnnotation = kotlinType.getAnnotations().mo1381findAnnotation(StandardNames.FqNames.parameterName);
        if (mo1381findAnnotation == null) {
            return null;
        }
        Object singleOrNull = CollectionsKt.singleOrNull(mo1381findAnnotation.getAllValueArguments().values());
    ...
    public static final List<TypeProjection> getFunctionTypeArgumentProjections(KotlinType kotlinType, List<? extends KotlinType> contextReceiverTypes, List<? extends KotlinType> parameterTypes, List<Name> list, KotlinType returnType, KotlinBuiltIns builtIns) {
        ...
        for (Object obj : parameterTypes) {
            ...
            if (name != null) {
                ...
                kotlinType2 = TypeUtilsKt.replaceAnnotations(kotlinType2, Annotations.Companion.create(CollectionsKt.plus(kotlinType2.getAnnotations(), new BuiltInAnnotationDescriptor(builtIns, fqName, MapsKt.mapOf(TuplesKt.to(identifier, new StringValue(asString))), false, 8, null))));
            }
