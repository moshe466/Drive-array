                    ...
                    try {
                        if (str.hashCode() == -1307105544 && str.equals("activateSystemCursor")) {
                            try {
                                ...
                                result.error(Constants.IPC_BUNDLE_KEY_SEND_ERROR, "Error when setting cursors: " + e4.getMessage(), null);
                            }
                        }
                    } catch (Exception e5) {
                        result.error(Constants.IPC_BUNDLE_KEY_SEND_ERROR, "Unhandled error: " + e5.getMessage(), null);
                    }
                }
            }
        };
