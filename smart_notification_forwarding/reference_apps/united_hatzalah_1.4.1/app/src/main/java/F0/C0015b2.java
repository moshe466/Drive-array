package F0;

import android.R;
import android.content.Context;
import android.net.ConnectivityManager;
import android.net.Network;
import android.net.NetworkCapabilities;
import com.google.android.gms.fido.fido2.api.common.UserVerificationMethods;
import java.nio.charset.Charset;
import java.nio.charset.StandardCharsets;
import java.util.Objects;
import org.apache.tika.fork.ForkServer;

/* renamed from: F0.b2, reason: case insensitive filesystem */
/* loaded from: classes.dex */
public final class C0015b2 extends AbstractC0060n {
    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public C0015b2(C0093v1 c0093v1, C0054l1 c0054l1) {
        super(c0093v1, c0054l1);
        byte[] bArr = {43, -1, 30, -54, -122, 53};
        byte[] bArr2 = new byte[8];
        bArr2[0] = 59;
        bArr2[1] = -94;
        bArr2[579396848 ^ ((((~C0015b2.class.getName().length()) | (-536895569)) - (-578839635)) + ((C0015b2.class.getName().length() & 536895696) | 557216))] = -57;
        bArr2[3] = 3;
        bArr2[4] = -29;
        bArr2[5] = 71;
        bArr2[6] = -16;
        bArr2[7] = 46;
        z(bArr, bArr2);
        Charset charset = StandardCharsets.UTF_8;
        new String(bArr, charset).intern();
        byte[] bArr3 = {-96, -55, 65, 14, -77, 100, -97, -124};
        z(bArr3, new byte[]{-74, -102, -110, -53, -85, 51, 126, 84});
        new String(bArr3, charset).intern();
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:3:0x0135. Please report as an issue. */
    public static void k(byte[] bArr, byte[] bArr2) {
        int length;
        int i;
        int length2;
        int i3;
        int i4;
        int i5;
        int i6;
        int i7 = ~C0015b2.class.getName().length();
        int length3 = (((~(((C0015b2.class.getName().length() | 70245657) | i7) - (i7 | (C0015b2.class.getName().length() & (-70245658))))) & (-1979440632)) + ((C0015b2.class.getName().length() & 1074528264) | 1093142560)) ^ (-886298072);
        int f4 = AbstractC0008a.f(C0015b2.class, -1);
        int length4 = (((f4 | (-1789924155)) - ((21884101 | f4) ^ (-1811767295))) + (((C0015b2.class.getName().length() | 1811808253) - 1811808253) | 537399298)) ^ (-1274367997);
        int length5 = ((((~C0015b2.class.getName().length()) | (-576567005)) & 276971586) + ((C0015b2.class.getName().length() & 36928) | 1073844225)) ^ 1350815811;
        int length6 = ((((~C0015b2.class.getName().length()) | (-1157759625)) & 1755853004) + ((C0015b2.class.getName().length() & 1073973402) | (-2146202606))) ^ (-390349602);
        int i8 = ((~C0015b2.class.getName().length()) | (-529537184)) & 457019905;
        int length7 = C0015b2.class.getName().length();
        int i9 = (-1686268015) ^ ((((454038545 & length7) ^ (-2143287920)) + (length7 & 1040)) + i8);
        int length8 = ((((~C0015b2.class.getName().length()) | (-1064961)) + 689325073) + ((C0015b2.class.getName().length() & (-2112862208)) | (-2109732696))) ^ (-1420407624);
        int i10 = ((~C0015b2.class.getName().length()) | 91711000) & (-1070824876);
        int length9 = C0015b2.class.getName().length();
        int i11 = (i10 + (9457696 | ((length9 | (-1064779676)) - (length9 ^ (-1064779676))))) ^ 1492981618;
        short[] sArr = null;
        while (true) {
            switch (i11) {
                case -2143294076:
                    int i12 = ~C0015b2.class.getName().length();
                    if (length3 < length4) {
                        int length10 = (C0015b2.class.getName().length() & 268439810) | 285217280;
                        int i13 = -((i12 | (-1553600102)) - (((-1553600360) | i12) ^ 536887698));
                        i5 = (((~i13) & length10) * 2) - (i13 ^ length10);
                        i6 = -1524017045;
                        i11 = i6 ^ i5;
                    } else {
                        length = ((i12 | (-747233512)) & (-1862204400)) + ((C0015b2.class.getName().length() & 1073807362) | 1116733474);
                        i = -375509041;
                        i11 = length ^ i;
                    }
                case -2038999444:
                    int i14 = ~C0015b2.class.getName().length();
                    int length11 = (161497089 & (((((C0015b2.class.getName().length() & (~i14)) & 797295576) + 797295576) + i14) - ((C0015b2.class.getName().length() | i14) & 797295576))) + ((C0015b2.class.getName().length() & (-2145386455)) | (-2147483476));
                    int a2 = ((short) ((length5 << E1.a(length11 | (-1985986391), -1985986391, length11)) + sArr[((((~C0015b2.class.getName().length()) | (-1085986263)) & 1078327440) + ((C0015b2.class.getName().length() & 1612763792) | 674234944)) ^ 1752562386])) ^ (length5 + i9);
                    int i15 = ~C0015b2.class.getName().length();
                    int length12 = length5 >>> ((((~(((C0015b2.class.getName().length() | 626856794) | i15) - ((C0015b2.class.getName().length() & (-626856795)) | i15))) & 957405457) + ((C0015b2.class.getName().length() & 588787984) | 36185216)) ^ 993590676);
                    short s3 = sArr[((((~C0015b2.class.getName().length()) | 1248713193) & 826417528) + ((C0015b2.class.getName().length() & 822288912) | (-2138488320))) ^ (-1312070789)];
                    int i16 = -length12;
                    int i17 = i16 | s3;
                    int i18 = (i17 - (i16 * 2)) + ((i16 ^ s3) ^ i17);
                    int i19 = -B0.a(i18 | (~a2), i18 - a2);
                    length6 = (short) D.a(length6, 3, -(D1.a(length6, i19) | (i19 & 2)), 1);
                    int i20 = ((~C0015b2.class.getName().length()) | (-549847554)) + 1624126210;
                    int length13 = (C0015b2.class.getName().length() & 549848649) | 67175498;
                    length5 = (short) (length5 - ((((short) ((length6 << (1691301711 ^ ((length13 & i20) + (i20 | length13)))) + sArr[((((~C0015b2.class.getName().length()) | (-1005965450)) & 153223237) + ((C0015b2.class.getName().length() & 220201009) | 335544368)) ^ 488767605])) ^ (((i9 | length6) - ((C0015b2.class.getName().length() & (~length6)) & i9)) + ((C0015b2.class.getName().length() | length6) & i9))) ^ ((length6 >>> (((((~C0015b2.class.getName().length()) | (-30261291)) & (-1534000062)) + ((C0015b2.class.getName().length() & 8609814) | 2285588)) ^ (-1531714477))) + sArr[((((~C0015b2.class.getName().length()) | (-23496740)) & 827084804) + ((C0015b2.class.getName().length() & (-2117787632)) | (-2139021104))) ^ (-1311936299)])));
                    int i21 = ((~C0015b2.class.getName().length()) | (-412319609)) & (-1959782776);
                    int length14 = (C0015b2.class.getName().length() & 403838542) | 268582982;
                    int i22 = -i21;
                    int i23 = (((~i22) & length14) * 2) - (i22 ^ length14);
                    i9 = (short) U0.a(1691170566 & i23, (-1691170567) - i23, i9);
                    length8++;
                    length = (((~C0015b2.class.getName().length()) | (-961655275)) & 25184460) + ((C0015b2.class.getName().length() & 150995145) | 140771329);
                    i = 1965034008;
                    i11 = length ^ i;
                case -1809249287:
                    byte b4 = bArr[(((((~C0015b2.class.getName().length()) | 1233459797) & 125923146) + ((C0015b2.class.getName().length() & 774137098) | 674496513)) ^ 800419659) + length3];
                    int length15 = ((((~C0015b2.class.getName().length()) | (-7107622)) & 402932290) + ((C0015b2.class.getName().length() & 546586672) | 546340912)) ^ 949273229;
                    int length16 = ((C0015b2.class.getName().length() | length15) - (b4 | length15)) + I2.a(C0015b2.class, b4) + (C0015b2.class.getName().length() & length15);
                    int length17 = ((((~C0015b2.class.getName().length()) | (-81143879)) & 438583424) + ((C0015b2.class.getName().length() & 786435) | 8921603)) ^ 447505026;
                    byte b5 = bArr[((length17 & length3) * 2) + (length17 ^ length3)];
                    int i24 = ~C0015b2.class.getName().length();
                    length5 = (short) (((b5 & ((-1954201202) ^ ((((C0015b2.class.getName().length() | (-2105278367)) - (i24 | (-1545180443))) + (I2.a(C0015b2.class, 568748773 | i24) + (C0015b2.class.getName().length() & (-2105278367)))) + ((C0015b2.class.getName().length() & (-2097135360)) | 151077136)))) << (((((~C0015b2.class.getName().length()) | (-1592082969)) & 140665109) + ((C0015b2.class.getName().length() & 142103568) | 1612800)) ^ 142277917)) | length16);
                    int i25 = ~C0015b2.class.getName().length();
                    int length18 = (-1901610175) ^ ((((((~i25) & (-569955033)) + i25) | 2038255548) - 2038255548) + ((C0015b2.class.getName().length() & 144806464) | 136645376));
                    int i26 = -length3;
                    int i27 = i26 | length18;
                    byte b6 = bArr[(i27 - (i26 * 2)) + ((length18 ^ i26) ^ i27)];
                    int i28 = (((-199685676) | r7) - 1591672428) - ((~C0015b2.class.getName().length()) | (-180811308));
                    int length19 = (C0015b2.class.getName().length() & 23072776) | 272636008;
                    int length20 = b6 & ((-1319036669) ^ (((length19 | i28) - ((C0015b2.class.getName().length() & (~i28)) & length19)) + (length19 & (i28 | C0015b2.class.getName().length()))));
                    int i29 = ((~C0015b2.class.getName().length()) | (-1009031633)) & 545538049;
                    int length21 = (C0015b2.class.getName().length() & 537143360) | 10560;
                    int length22 = bArr[(545548610 ^ ((length21 & i29) + (i29 | length21))) + length3] & (((((~C0015b2.class.getName().length()) | 75364313) & 1242301609) + ((C0015b2.class.getName().length() & 1249907040) | (-1602217664))) ^ (-359916266));
                    int length23 = C0015b2.class.getName().length();
                    length6 = (short) (length20 | (length22 << ((((1779401364 | (((~length23) - length23) + length23)) & 447961710) + ((C0015b2.class.getName().length() & (-1313580806)) | (-519831408))) ^ (-71869706))));
                    int i30 = ~C0015b2.class.getName().length();
                    i9 = 758110381 ^ (((((-1343875612) | i30) + 311432716) - (i30 | (-1074391060))) + ((C0015b2.class.getName().length() & 273678921) | (-1069545407)));
                    int i31 = ~C0015b2.class.getName().length();
                    int length24 = 1409942802 & (((((C0015b2.class.getName().length() & (~i31)) & 91135407) + 91135407) + i31) - ((i31 | C0015b2.class.getName().length()) & 91135407));
                    int length25 = (C0015b2.class.getName().length() & (-804257776)) | (-2094006112);
                    int i32 = -length24;
                    length8 = (-684063310) ^ (((~i32) & length25) - (i32 & (~length25)));
                    length2 = (((~C0015b2.class.getName().length()) | (-537919489)) - (-806798471)) + ((C0015b2.class.getName().length() & 674768897) | 153626665);
                    i3 = 1174056570 - length2;
                    i4 = -1174056571;
                    i11 = ((length2 & i4) * 2) + i3;
                case -1740520186:
                    sArr = new short[((((~C0015b2.class.getName().length()) | (-382746167)) & 102532165) + ((C0015b2.class.getName().length() & 105907748) | 4198960)) ^ 106731121];
                    length3 = ((((~C0015b2.class.getName().length()) | (-6036961)) & 1233145505) + ((C0015b2.class.getName().length() & 809508000) | 809603328)) ^ 2042748833;
                    int i33 = ((~C0015b2.class.getName().length()) | 1688058452) & 872484865;
                    int length26 = C0015b2.class.getName().length() & 268460041;
                    i5 = (((((C0015b2.class.getName().length() & (~length26)) & 4218888) + 4218888) + length26) - ((length26 | C0015b2.class.getName().length()) & 4218888)) + i33;
                    i6 = 434661073;
                    i11 = i6 ^ i5;
                case -1489518479:
                    int length27 = C0015b2.class.getName().length();
                    int length28 = (((-2053077912) & ((516782023 - length27) + (((-((-1) - length27)) - 1) | (-516782024)))) + ((C0015b2.class.getName().length() & (-1054752728)) | 1073823745)) ^ (-979254165);
                    int length29 = bArr2[(((~length3) & length28) * ((~length28) & length3)) + ((length28 & length3) * (length28 | length3))] & (((((~C0015b2.class.getName().length()) | (-1883938358)) & (-738125179)) + ((C0015b2.class.getName().length() & 1343232517) | 546308360)) ^ (-191816846));
                    int i34 = ~C0015b2.class.getName().length();
                    int i35 = 73539736 & (((~i34) & (-1772650326)) + i34);
                    int length30 = (C0015b2.class.getName().length() & 35664144) | 33608448;
                    int i36 = -i35;
                    byte b7 = bArr2[((107148186 ^ ((((~i36) & length30) * 2) - (i36 ^ length30))) * length3) + ((((I2.a(C0015b2.class, -1) | (-532481)) - (-67641369)) + ((C0015b2.class.getName().length() & 532546) | 1602)) ^ 67642971)];
                    int i37 = ~C0015b2.class.getName().length();
                    int length31 = (b7 & (((663757504 & ((i37 + 1314070430) - (i37 & 1314070430))) + ((C0015b2.class.getName().length() & 834674756) | 272630796)) ^ 936388147)) << ((((I2.a(C0015b2.class, -1) | (-33554434)) - (-1107366402)) + ((C0015b2.class.getName().length() & (-2113929151)) | (-2147475136))) ^ (-1040108727));
                    sArr[length3] = (short) ((length31 ^ length29) + (length29 & length31));
                    length3++;
                    length = ((I2.a(C0015b2.class, -1) | (-167014194)) & 1157999680) + ((C0015b2.class.getName().length() & 159661328) | (-2004872944));
                    i = -533943416;
                    i11 = length ^ i;
                case -473033593:
                    int i38 = -length3;
                    int i39 = -bArr.length;
                    int i40 = i39 | i38;
                    int i41 = (i40 - (i39 * 2)) + ((i39 ^ i38) ^ i40);
                    byte b8 = bArr[bArr.length - length3];
                    int length32 = C0015b2.class.getName().length();
                    bArr[i41] = (byte) (b8 ^ bArr2[length3 % (((((-878819395) | ((length32 - 1) - (length32 * 2))) & 1490255976) + ((C0015b2.class.getName().length() & 274827331) | 556017667)) ^ 2046273635)]);
                    length3--;
                    int f5 = (AbstractC0008a.f(C0015b2.class, -1) | 114408723) & 1183666176;
                    int length33 = C0015b2.class.getName().length() & 1074544770;
                    length = S.a(length33, (-268567684) | ((-length33) - 1), 268567684, f5);
                    i = 836032333;
                    i11 = length ^ i;
                case 766056152:
                    int i42 = ((~C0015b2.class.getName().length()) | (-889871025)) & 1233748555;
                    int length34 = C0015b2.class.getName().length();
                    int i43 = (length34 + 84675108) - (length34 | 84675108);
                    if (length3 < (1842188139 ^ ((((~i43) & 608439588) + i43) + i42))) {
                        int i44 = ((~C0015b2.class.getName().length()) | 1878725846) & 1912684595;
                        int length35 = (C0015b2.class.getName().length() & 268589089) | 661640;
                        length = AbstractC0099x.a(i44 | length35, 2, (~i44) ^ length35);
                        i = -717449014;
                    } else {
                        length = (((~C0015b2.class.getName().length()) | (-1477955618)) & (-1604246503)) + ((C0015b2.class.getName().length() & 1074350177) | 1342720098);
                        i = -887872332;
                    }
                    i11 = length ^ i;
                case 974072829:
                    int length36 = bArr.length;
                    int i45 = ((~C0015b2.class.getName().length()) | 1711185063) & 170281206;
                    int length37 = (C0015b2.class.getName().length() & 251684176) | 1694512896;
                    int i46 = -i45;
                    length3 = length36 % (1864794098 ^ (((~i46) & length37) - (i46 & (~length37))));
                    length = (((~C0015b2.class.getName().length()) | 991120067) & (-2113137661)) + ((C0015b2.class.getName().length() & (-1878240248)) | 285229064);
                    i = -195569723;
                    i11 = length ^ i;
                case 998066383:
                    length3 = (((AbstractC0008a.f(C0015b2.class, -1) | 314136709) & 371231304) + (((C0015b2.class.getName().length() | (-67142233)) + 67142233) | (-1996488432))) ^ (-1625257128);
                    length4 = bArr.length - (bArr.length % (((((~C0015b2.class.getName().length()) | 366661365) & 1344150018) + ((C0015b2.class.getName().length() & (-1006333853)) | (-2080341919))) ^ (-736191897)));
                    length = (((~C0015b2.class.getName().length()) | (-1359635359)) & 49026131) + ((C0015b2.class.getName().length() & (-1860698094)) | (-1190123008));
                    i = 1002689495;
                    i11 = length ^ i;
                case 1314339506:
                    break;
                case 1734050766:
                    int i47 = ~C0015b2.class.getName().length();
                    if (length3 > 0) {
                        int length38 = C0015b2.class.getName().length();
                        length = ((i47 | (-268772210)) & 282132586) + (168323072 | ((length38 + 402735200) - (length38 | 402735200)));
                        i = -115901203;
                        i11 = length ^ i;
                    } else {
                        int length39 = (C0015b2.class.getName().length() & R.^attr-private.__removed0) | 553664516;
                        int i48 = -((i47 | 1510858717) & 403833600);
                        i5 = ((~i48) & length39) - (i48 & (~length39));
                        i6 = 2001041846;
                        i11 = i6 ^ i5;
                    }
                case 1771480224:
                    bArr[(((((~C0015b2.class.getName().length()) | 1110430873) & 1241612298) + ((C0015b2.class.getName().length() & 150996226) | 84419840)) ^ 1326032138) + length3] = (byte) ((((((~C0015b2.class.getName().length()) | 1603962366) & 25199440) + (((C0015b2.class.getName().length() | (-1311235)) + 1311235) | (-2146172766))) ^ (-2120973555)) & length5);
                    int length40 = (((((~C0015b2.class.getName().length()) | (-1388708984)) & 706816128) + ((C0015b2.class.getName().length() & 1124204552) | 1363312648)) ^ 2070128777) + length3;
                    int i49 = ((~C0015b2.class.getName().length()) | 367288948) & 548745488;
                    int length41 = C0015b2.class.getName().length();
                    bArr[length40] = (byte) ((length5 >> ((i49 + (21135364 | ((length41 + 558960896) - (length41 | 558960896)))) ^ 569880860)) & (((((~C0015b2.class.getName().length()) | 2113158628) & 1026558002) + ((C0015b2.class.getName().length() & 8392730) | 8525645)) ^ 1035083648));
                    int length42 = (((~C0015b2.class.getName().length()) | 715175224) & 136512788) + ((C0015b2.class.getName().length() & 196644) | (-2146430752));
                    int a4 = AbstractC0105y1.a((~length42) | (-2009917962), (-2009917962) - length42, length3);
                    int i50 = ((~C0015b2.class.getName().length()) | (-1010633609)) & 678986012;
                    int length43 = C0015b2.class.getName().length();
                    int i51 = ~(((951583497 & length43) + 276825601) - (length43 & 276824577));
                    int i52 = -i50;
                    bArr[a4] = (byte) ((V2.a(~i52, i51, (i51 + i52) + 1) ^ 955811810) & length6);
                    int length44 = (((((~C0015b2.class.getName().length()) | (-1084937228)) & 438503696) + ((C0015b2.class.getName().length() & 69369860) | (-2080078843))) ^ (-1641575146)) + length3;
                    int i53 = ~C0015b2.class.getName().length();
                    int length45 = length6 >> (2092810490 ^ ((((C0015b2.class.getName().length() | 674349280) - (i53 | 1869872636)) + (AbstractC0008a.f(C0015b2.class, 1197735420 | i53) + (C0015b2.class.getName().length() & 674349280))) + ((C0015b2.class.getName().length() & 1754529808) | 1418461202)));
                    int i54 = ((~C0015b2.class.getName().length()) | 1601418652) & 1439188132;
                    int length46 = (C0015b2.class.getName().length() & 545800290) | (-1442676670);
                    int i55 = -i54;
                    bArr[length44] = (byte) (length45 & ((-3488743) ^ (((~i55) & length46) - (i55 & (~length46)))));
                    length3 += 4;
                    length = (((~C0015b2.class.getName().length()) | (-171976913)) & 318775824) + ((C0015b2.class.getName().length() & 33562640) | 136194);
                    i = -1824662634;
                    i11 = length ^ i;
                case 2093236949:
                    if (length8 < (((((~C0015b2.class.getName().length()) | (-616910267)) & 1303391760) + ((C0015b2.class.getName().length() & 75500825) | 537198861)) ^ 1840590653)) {
                        length2 = (((~C0015b2.class.getName().length()) | 1297715640) & 556926729) + ((C0015b2.class.getName().length() & 874653185) | 335552516);
                        i3 = (-1287294623) - length2;
                        i4 = 1287294622;
                        i11 = ((length2 & i4) * 2) + i3;
                    } else {
                        int i56 = ~C0015b2.class.getName().length();
                        length = (1141965102 & ((-1207265904) + i56 + (((-i56) - 1) | 1207265904))) + ((C0015b2.class.getName().length() & 1292960864) | 150996032);
                        i = 612868558;
                        i11 = length ^ i;
                    }
                default:
                    int i57 = ~C0015b2.class.getName().length();
                    int i58 = (((-313266948) | i57) + 45165696) - (i57 | (-269226756));
                    length = D.a(i58, 3, -D1.a(i58, (C0015b2.class.getName().length() & 44040224) | (-1811807712)), 1);
                    i = -361272203;
                    i11 = length ^ i;
            }
            return;
        }
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:3:0x0048. Please report as an issue. */
    public static void r(byte[] bArr, byte[] bArr2) {
        int i;
        int i3;
        byte[] bArr3 = null;
        int i4 = 1516727821;
        int i5 = 0;
        int i6 = 0;
        int i7 = 0;
        byte[] bArr4 = null;
        while (true) {
            int i8 = ((i4 & 16777216) * (i4 | 16777216)) + ((i4 & (-16777217)) * ((~i4) & 16777216));
            int i9 = i4 >>> 8;
            int a2 = j3.a(650911840 & (~i8) & i9, i9, i8, (i8 | 650911840) & i9);
            int i10 = (a2 ^ 642535957) + ((a2 & 642535957) * 2);
            int i11 = -365117735;
            boolean z3 = true;
            switch (((~i10) + ((i10 | 1) * 2)) ^ 962785775) {
                case -1896910703:
                    int length = bArr4.length;
                    int i12 = 0 - i5;
                    int i13 = (length ^ i12) + ((length & i12) * 2);
                    byte b4 = bArr3[i13];
                    int length2 = bArr4.length;
                    int i14 = 0 - i12;
                    int i15 = i14 | length2;
                    byte b5 = bArr3[D.a(i14, 2, i15, (length2 ^ i14) ^ i15)];
                    bArr3[i13] = (byte) (((byte) (((byte) 2) * ((byte) (b5 | b4)))) - ((byte) (b5 ^ b4)));
                    i4 = -746753280;
                case -1725904394:
                    i7 = bArr4.length % 4;
                    if ((((i7 > 1 ? 1 : (i7 == 1 ? 0 : -1)) >>> 31) & 1) == 0) {
                        i4 = -365117735;
                    } else {
                        i4 = -458924450;
                    }
                case -1399959314:
                    int a4 = j3.a((-1205100636) & i6, i6, 3, (-1205100633) & i6);
                    byte b6 = bArr3[a4];
                    int i16 = ((b6 & ForkServer.DONE) * (b6 | ForkServer.DONE)) + ((b6 & ForkServer.ERROR) * ((~b6) & 16777216));
                    int i17 = i6 - 1;
                    int i18 = i17 - (i6 | (-3));
                    int i19 = bArr3[i18] & ForkServer.ERROR;
                    int i20 = i19 * ((~i19) & 65536);
                    int a5 = S.a(i20, i16, 1, ((-1) - i20) | ((-1) - i16));
                    int i21 = i17 - (i6 | (-2));
                    int i22 = bArr3[i21] & ForkServer.ERROR;
                    int i23 = i22 * ((~i22) & UserVerificationMethods.USER_VERIFY_HANDPRINT);
                    int i24 = (i23 - 1) - ((~a5) | i23);
                    int i25 = bArr3[i6] & ForkServer.ERROR;
                    int a6 = S.a(i24, i25, 1, ((-1) - i24) | ((-1) - i25));
                    byte b7 = bArr4[a4];
                    int i26 = ((b7 & ForkServer.DONE) * (b7 | ForkServer.DONE)) + ((b7 & ForkServer.ERROR) * ((~b7) & 16777216));
                    int i27 = bArr4[i18] & ForkServer.ERROR;
                    int i28 = ((i27 * ((~i27) & 65536)) & (~i26)) + i26;
                    int i29 = bArr4[i21] & ForkServer.ERROR;
                    int i30 = i29 * ((~i29) & UserVerificationMethods.USER_VERIFY_HANDPRINT);
                    int i31 = ~((((~i30) | 911399251) | i28) - ((i30 & 911399251) | i28));
                    int i32 = bArr4[i6] & ForkServer.ERROR;
                    int i33 = ~((((~i31) | 1433568692) | i32) - ((i31 & 1433568692) | i32));
                    int i34 = a6 << ((a6 > Double.NaN ? 1 : (a6 == Double.NaN ? 0 : -1)) >>> 31);
                    int i35 = (-1254002618) - ((i34 & 2) | ((-1672003491) - i34));
                    int i36 = (i35 + i33) - ((i35 & i33) * 2);
                    bArr4[i6] = (byte) i36;
                    bArr4[i21] = (byte) (i36 >>> 8);
                    bArr4[i18] = (byte) (i36 >>> 16);
                    bArr4[a4] = (byte) (i36 >>> 24);
                    i6 = (i6 ^ 4) + ((i6 & 4) * 2);
                    int length3 = bArr4.length;
                    int length4 = 0 - (bArr4.length % 4);
                    int i37 = ((i6 > AbstractC0014b1.a((length3 & 2) | D1.a(length4, length3), length4 * 3) ? 1 : (i6 == AbstractC0014b1.a((length3 & 2) | D1.a(length4, length3), length4 * 3) ? 0 : -1)) >>> 31) & 1;
                    if (i37 != 0) {
                        i = -1605440657;
                    } else {
                        i = -365117735;
                    }
                    if (i37 != 0) {
                        i4 = i;
                    } else {
                        i4 = -169475207;
                    }
                case -1135475043:
                    break;
                case 180635757:
                    int length5 = bArr.length;
                    int length6 = 0 - (bArr.length % 4);
                    if ((length5 ^ (~length6)) + ((length5 | length6) * 2) + 1 <= 0) {
                        z3 = false;
                    }
                    if (z3) {
                        i3 = -1605440657;
                    } else {
                        i3 = -365117735;
                    }
                    if (z3) {
                        i4 = i3;
                    } else {
                        i4 = -169475207;
                    }
                    bArr3 = bArr2;
                    bArr4 = bArr;
                    i6 = 0;
                case 511524454:
                    int length7 = bArr4.length;
                    int i38 = 0 - i5;
                    int i39 = 0 - i38;
                    int i40 = ((~length7) & i39) * 2;
                    int length8 = bArr4.length;
                    byte b8 = bArr4[((length8 | i38) * 2) - (length8 ^ i38)];
                    int length9 = bArr4.length;
                    byte b9 = bArr3[(i38 ^ length9) + ((length9 & i38) * 2)];
                    bArr4[(length7 ^ i39) - i40] = (byte) (((byte) (b9 - b8)) + ((byte) (((byte) 2) * ((byte) ((~b9) & b8)))));
                    i7 = AbstractC0099x.a(i5, 3, (~i5) * 2);
                    if ((((i5 > 2 ? 1 : (i5 == 2 ? 0 : -1)) >>> 31) & 1) == 0) {
                        i4 = -365117735;
                    } else {
                        i4 = -458924450;
                    }
                case 961838909:
                    int length10 = bArr4.length;
                    int i41 = 0 - i7;
                    if ((bArr3[((length10 | i41) - ((165327505 & (~i41)) & length10)) + ((i41 | 165327505) & length10)] > Double.NaN ? 1 : (bArr3[((length10 | i41) - ((165327505 & (~i41)) & length10)) + ((i41 | 165327505) & length10)] == Double.NaN ? 0 : -1)) <= -1) {
                        z3 = false;
                    }
                    if (!z3) {
                        i11 = 1093626513;
                    }
                    if (z3) {
                        i4 = -746753280;
                    } else {
                        i4 = i11;
                    }
                    i5 = i7;
                default:
                    i4 = -365117735;
            }
            return;
        }
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:3:0x003b. Please report as an issue. */
    public static void x(byte[] bArr, byte[] bArr2) {
        byte[] bArr3 = null;
        int i = 0;
        int i3 = 0;
        int i4 = -1850458006;
        byte[] bArr4 = null;
        while (true) {
            int i5 = ((16777216 & i4) * (i4 | 16777216)) + (((-16777217) & i4) * ((~i4) & 16777216));
            int i6 = i4 >>> 8;
            int i7 = (~i5) | i6;
            boolean z3 = true;
            int i8 = (i6 - 1) - i7;
            int i9 = (-1700147435) - ((i8 & 2) | (2028104049 - i8));
            int i10 = -1396193641;
            switch ((-1363443157) ^ ((~i9) + ((i9 | 1) * 2))) {
                case -1940167324:
                    byte b4 = bArr3[i];
                    int i11 = ((byte) 0) - b4;
                    bArr3[i] = (byte) (((byte) (b4 & (~i11))) - ((byte) ((~b4) & i11)));
                    i4 = 614229416;
                case -360299937:
                    if ((bArr3[i3] > Double.NaN ? 1 : (bArr3[i3] == Double.NaN ? 0 : -1)) <= -1) {
                        z3 = false;
                    }
                    if (!z3) {
                        i10 = 427928065;
                    }
                    if (z3) {
                        i4 = 614229416;
                    } else {
                        i4 = i10;
                    }
                    i = i3;
                case 399486784:
                    break;
                case 585276366:
                    if (bArr.length <= 0) {
                        i4 = -1396193641;
                    } else {
                        i4 = 1985663266;
                    }
                    bArr4 = bArr;
                    bArr3 = bArr2;
                    i3 = 0;
                case 1733787683:
                    byte b5 = bArr4[i];
                    byte b6 = bArr3[i];
                    bArr4[i] = (byte) (((byte) (b6 + b5)) - ((byte) (((byte) 2) * ((byte) (b6 & b5)))));
                    i3 = (i ^ 1) + ((i & 1) * 2);
                    if ((((i3 > bArr4.length ? 1 : (i3 == bArr4.length ? 0 : -1)) >>> 31) & 1) == 0) {
                        i4 = -1396193641;
                    } else {
                        i4 = 1985663266;
                    }
                default:
                    i4 = -1396193641;
            }
            return;
        }
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:3:0x003f. Please report as an issue. */
    public static void z(byte[] bArr, byte[] bArr2) {
        byte[] bArr3 = null;
        int i = 0;
        int i3 = 0;
        int i4 = 0;
        int i5 = -894652659;
        byte[] bArr4 = null;
        while (true) {
            int i6 = ((i5 & 16777216) * (i5 | 16777216)) + ((i5 & (-16777217)) * ((~i5) & 16777216));
            int i7 = i5 >>> 8;
            int i8 = (i7 + i6) - (i7 & i6);
            int i9 = (i8 ^ 1458005263) + ((i8 & 1458005263) * 2);
            int i10 = 145880015;
            int i11 = 1298988808;
            boolean z3 = true;
            switch ((i9 - 1434379843) + (((~i9) & 1434379843) * 2)) {
                case -1970406716:
                    int length = bArr4.length;
                    int i12 = 0 - i;
                    int i13 = ~i12;
                    int i14 = ((length | i12) - ((602749225 & i13) & length)) + ((i12 | 602749225) & length);
                    byte b4 = bArr3[i14];
                    int length2 = bArr4.length;
                    byte b5 = bArr3[(length2 ^ i13) + ((i12 | length2) * 2) + 1];
                    int i15 = ((byte) 0) - b4;
                    bArr3[i14] = (byte) (((byte) (((byte) 2) * ((byte) (b5 & (~i15))))) - ((byte) (b5 ^ i15)));
                    i5 = -34715366;
                case -1882653318:
                    int i16 = (i3 - 1) - (i3 | (-4));
                    byte b6 = bArr3[i16];
                    int i17 = ((b6 & ForkServer.DONE) * (b6 | ForkServer.DONE)) + ((b6 & ForkServer.ERROR) * ((~b6) & 16777216));
                    int i18 = i3 + 3 + (((-1) - i3) | (-3));
                    int i19 = bArr3[i18] & ForkServer.ERROR;
                    int i20 = i19 * ((~i19) & 65536);
                    int i21 = ~((i17 | ((~i20) | 1169991170)) - ((i20 & 1169991170) | i17));
                    int a2 = j3.a(689061172 & i3, i3, 1, 689061173 & i3);
                    int i22 = bArr3[a2] & ForkServer.ERROR;
                    int i23 = ((~i21) & (i22 * ((~i22) & UserVerificationMethods.USER_VERIFY_HANDPRINT))) + i21;
                    int i24 = (i23 - 1) - ((~(bArr3[i3] & ForkServer.ERROR)) | i23);
                    byte b7 = bArr4[i16];
                    int i25 = ((b7 & ForkServer.DONE) * (b7 | ForkServer.DONE)) + ((b7 & ForkServer.ERROR) * ((~b7) & 16777216));
                    int i26 = bArr4[i18] & ForkServer.ERROR;
                    int i27 = i26 * ((~i26) & 65536);
                    int i28 = ~((i25 | ((~i27) | (-445685625))) - ((i27 & (-445685625)) | i25));
                    int i29 = bArr4[a2] & ForkServer.ERROR;
                    int i30 = i29 * ((~i29) & UserVerificationMethods.USER_VERIFY_HANDPRINT);
                    int i31 = (i30 + i28) - (i30 & i28);
                    int i32 = bArr4[i3] & ForkServer.ERROR;
                    int i33 = (i31 & (~i32)) + i32;
                    int i34 = i24 << ((i24 > Double.NaN ? 1 : (i24 == Double.NaN ? 0 : -1)) >>> 31);
                    int i35 = (i34 + i33) - ((i34 & i33) * 2);
                    int i36 = 659933421 - ((i35 & 2) | ((-1983400303) - i35));
                    bArr4[i3] = (byte) i36;
                    bArr4[a2] = (byte) (i36 >>> 8);
                    bArr4[i18] = (byte) (i36 >>> 16);
                    bArr4[i16] = (byte) (i36 >>> 24);
                    i3 = (i3 ^ 4) + ((i3 & 4) * 2);
                    int length3 = bArr4.length;
                    int length4 = 0 - (bArr4.length % 4);
                    int i37 = ((i3 > ((length3 ^ length4) + ((length3 & length4) * 2)) ? 1 : (i3 == ((length3 ^ length4) + ((length3 & length4) * 2)) ? 0 : -1)) >>> 31) & 1;
                    if (i37 != 0) {
                        i10 = 196573321;
                    }
                    if (i37 != 0) {
                        i5 = -826922365;
                    } else {
                        i5 = i10;
                    }
                case -625567707:
                    break;
                case 172635213:
                    int length5 = bArr4.length;
                    int i38 = 0 - i4;
                    if ((bArr3[(length5 ^ i38) + ((length5 & i38) * 2)] > Double.NaN ? 1 : (bArr3[(length5 ^ i38) + ((length5 & i38) * 2)] == Double.NaN ? 0 : -1)) <= -1) {
                        i5 = 196573321;
                    } else {
                        i5 = -34715366;
                    }
                    i = i4;
                case 614184219:
                    int length6 = bArr4.length;
                    int i39 = 0 - i;
                    int i40 = i39 * 3;
                    int a4 = D1.a(i39, length6);
                    int length7 = bArr4.length;
                    byte b8 = bArr4[(length7 ^ i39) + ((length7 & i39) * 2)];
                    int length8 = bArr4.length;
                    int i41 = 0 - i39;
                    byte b9 = bArr3[(((~i41) & length8) * 2) - (length8 ^ i41)];
                    bArr4[AbstractC0014b1.a((length6 & 2) | a4, i40)] = (byte) (((byte) (b9 + b8)) - ((byte) (((byte) 2) * ((byte) (b9 & b8)))));
                    i4 = ((-338014207) | i) + (338014206 | i);
                    int i42 = ((i > 2 ? 1 : (i == 2 ? 0 : -1)) >>> 31) & 1;
                    if (i42 != 0) {
                        i11 = 196573321;
                    }
                    if (i42 == 0) {
                        i5 = i11;
                    } else {
                        i5 = -518432968;
                    }
                case 835516413:
                    int length9 = bArr.length;
                    int length10 = 0 - (0 - (bArr.length % 4));
                    if ((length9 ^ length10) - (((~length9) & length10) * 2) <= 0) {
                        z3 = false;
                    }
                    if (z3) {
                        i10 = 196573321;
                    }
                    if (z3) {
                        i5 = -826922365;
                    } else {
                        i5 = i10;
                    }
                    bArr3 = bArr2;
                    bArr4 = bArr;
                    i3 = 0;
                case 1888416065:
                    i4 = bArr4.length % 4;
                    int i43 = ((i4 > 1 ? 1 : (i4 == 1 ? 0 : -1)) >>> 31) & 1;
                    if (i43 != 0) {
                        i11 = 196573321;
                    }
                    if (i43 == 0) {
                        i5 = i11;
                    } else {
                        i5 = -518432968;
                    }
                default:
                    i5 = 196573321;
            }
            return;
        }
    }

    public final void B(Context context) {
        char c4;
        char c5;
        char c6;
        byte b4;
        char c7;
        char c8;
        byte[] bArr = {107, 72, 28, 69, 78, -60, 109};
        k(bArr, new byte[]{70, 25, -68, 43, -107, -110, -65, -102});
        Charset charset = StandardCharsets.UTF_8;
        kotlin.jvm.internal.j.e(context, new String(bArr, charset).intern());
        C0031f2 n4 = T0.n(new C0016c(this, context, 9));
        byte[] bArr2 = {70, -85, 30, 76, -111, 79};
        AbstractC0060n.x(bArr2, new byte[]{52, -50, 109, 57, -3, 59, -77, -70});
        new String(bArr2, charset).intern();
        C0054l1 c0054l1 = this.f584f;
        C0047j2 c0047j2 = c0054l1.f573a;
        C0047j2 c0047j22 = c0054l1.f573a;
        c0047j2.getClass();
        byte[] bArr3 = {-56, 87, 56, 109, 51, -83, -114, -110, -20};
        AbstractC0060n.x(bArr3, new byte[]{-69, 46, 75, 25, 86, -64, -40, -62, -94});
        d(new String(bArr3, charset).intern(), n4);
        if (n4.b()) {
            c4 = 7;
            byte[] bArr4 = new byte[9];
            bArr4[0] = -123;
            bArr4[1] = 86;
            c5 = '\b';
            int i = ((~AbstractC0060n.class.getName().length()) | (-1642050844)) & 4226633;
            c6 = 6;
            c7 = 3;
            b4 = -123;
            c8 = 5;
            long j2 = 1098907650;
            long length = AbstractC0060n.class.getName().length() & 1094726667;
            long j3 = (((((((((j2 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j2 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j2 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j2 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)))) + (((((((((length >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((length >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((length >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))) + 6148914691236517205L;
            long j4 = (j3 >>> 48) & 43690;
            long j5 = ((j4 >>> 2) | (j4 >>> 1)) & 858993459;
            long j6 = ((j5 >>> 2) | j5) & 252645135;
            long j7 = (j3 >>> 32) & 43690;
            long j8 = ((j7 >>> 2) | (j7 >>> 1)) & 858993459;
            long j9 = ((j8 >>> 2) | j8) & 252645135;
            long j10 = ((((j9 >>> 4) | j9) & 16711935) << 16) + ((((j6 >>> 4) | j6) & 16711935) << 24);
            long j11 = (j3 >>> 16) & 43690;
            long j12 = ((j11 >>> 2) | (j11 >>> 1)) & 858993459;
            long j13 = ((j12 >>> 2) | j12) & 252645135;
            long j14 = j3 & 43690;
            long j15 = ((j14 >>> 2) | (j14 >>> 1)) & 858993459;
            long j16 = ((j15 >>> 2) | j15) & 252645135;
            bArr4[1103134281 ^ (i + ((int) ((((j16 >>> 4) | j16) & 16711935) + (((((j13 >>> 4) | j13) & 16711935) << 8) | j10))))] = 59;
            long j17 = -1;
            long length2 = AbstractC0060n.class.getName().length();
            long j18 = ((((((((j17 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((j17 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j17 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j17 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + (((((((((length2 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((length2 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((length2 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((length2 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))));
            long j19 = (j18 >>> 48) & 21845;
            long j20 = ((j19 >>> 1) | j19) & 858993459;
            long j21 = ((j20 >>> 2) | j20) & 252645135;
            long j22 = (j18 >>> 32) & 21845;
            long j23 = ((j22 >>> 1) | j22) & 858993459;
            long j24 = ((j23 >>> 2) | j23) & 252645135;
            long j25 = ((((j24 >>> 4) | j24) & 16711935) << 16) + ((((j21 >>> 4) | j21) & 16711935) << 24);
            long j26 = (j18 >>> 16) & 21845;
            long j27 = ((j26 >>> 1) | j26) & 858993459;
            long j28 = ((j27 >>> 2) | j27) & 252645135;
            long j29 = j18 & 21845;
            long j30 = ((j29 >>> 1) | j29) & 858993459;
            long j31 = ((j30 >>> 2) | j30) & 252645135;
            bArr4[3] = 1041982855 ^ ((((AbstractC0060n.class.getName().length() & 269116674) | 302661762) - (~((((int) ((((j31 >>> 4) | j31) & 16711935) + (((((j28 >>> 4) | j28) & 16711935) << 8) + j25))) | 1046300901) & 739321089))) - 1);
            long j32 = 2119225022;
            long j33 = ~AbstractC0060n.class.getName().length();
            long j34 = (((((((((j32 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j32 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((j32 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j32 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))) + ((((((((j33 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((j33 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j33 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j33 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + 6148914691236517205L;
            long j35 = (j34 >>> 48) & 43690;
            long j36 = ((j35 >>> 2) | (j35 >>> 1)) & 858993459;
            long j37 = ((j36 >>> 2) | j36) & 252645135;
            long j38 = (j34 >>> 32) & 43690;
            long j39 = ((j38 >>> 2) | (j38 >>> 1)) & 858993459;
            long j40 = ((j39 >>> 2) | j39) & 252645135;
            long j41 = ((((j40 >>> 4) | j40) & 16711935) << 16) + ((((j37 >>> 4) | j37) & 16711935) << 24);
            long j42 = (j34 >>> 16) & 43690;
            long j43 = ((j42 >>> 2) | (j42 >>> 1)) & 858993459;
            long j44 = ((j43 >>> 2) | j43) & 252645135;
            long j45 = j34 & 43690;
            long j46 = ((j45 >>> 2) | (j45 >>> 1)) & 858993459;
            long j47 = ((j46 >>> 2) | j46) & 252645135;
            int i3 = ((int) (((((j44 >>> 4) | j44) & 16711935) << 8) | j41 | (((j47 >>> 4) | j47) & 16711935))) & 1114670369;
            long j48 = 337650065;
            long length3 = AbstractC0060n.class.getName().length();
            long j49 = (((((((((j48 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j48 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j48 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j48 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + ((((((((length3 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + (((((((((length3 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((length3 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length3 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
            long j50 = (j49 >>> 48) & 43690;
            long j51 = ((j50 >>> 2) | (j50 >>> 1)) & 858993459;
            long j52 = ((j51 >>> 2) | j51) & 252645135;
            long j53 = (j49 >>> 32) & 43690;
            long j54 = ((j53 >>> 2) | (j53 >>> 1)) & 858993459;
            long j55 = ((j54 >>> 2) | j54) & 252645135;
            long j56 = ((((j55 >>> 4) | j55) & 16711935) << 16) | ((((j52 >>> 4) | j52) & 16711935) << 24);
            long j57 = (j49 >>> 16) & 43690;
            long j58 = ((j57 >>> 2) | (j57 >>> 1)) & 858993459;
            long j59 = ((j58 >>> 2) | j58) & 252645135;
            long j60 = ((((j59 >>> 4) | j59) & 16711935) << 8) + j56;
            long j61 = j49 & 43690;
            long j62 = ((j61 >>> 2) | (j61 >>> 1)) & 858993459;
            long j63 = (j62 | (j62 >>> 2)) & 252645135;
            int i4 = i3 + (((int) (((j63 | (j63 >>> 4)) & 16711935) + j60)) | 336142480);
            long j64 = -1450812894;
            long j65 = i4;
            long j66 = (((((((((j64 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j64 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((j64 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j64 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))) + (((((((((j65 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j65 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((j65 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j65 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
            long j67 = (j66 >>> 48) & 21845;
            long j68 = (j67 | (j67 >>> 1)) & 858993459;
            long j69 = (j68 | (j68 >>> 2)) & 252645135;
            long j70 = (j66 >>> 32) & 21845;
            long j71 = ((j70 >>> 1) | j70) & 858993459;
            long j72 = ((j71 >>> 2) | j71) & 252645135;
            long j73 = (((j69 | (j69 >>> 4)) & 16711935) << 24) | ((((j72 >>> 4) | j72) & 16711935) << 16);
            long j74 = (j66 >>> 16) & 21845;
            long j75 = ((j74 >>> 1) | j74) & 858993459;
            long j76 = ((j75 >>> 2) | j75) & 252645135;
            long j77 = ((((j76 >>> 4) | j76) & 16711935) << 8) + j73;
            long j78 = j66 & 21845;
            long j79 = (j78 | (j78 >>> 1)) & 858993459;
            long j80 = (j79 | (j79 >>> 2)) & 252645135;
            bArr4[4] = (int) (((j80 | (j80 >>> 4)) & 16711935) + j77);
            bArr4[5] = -3;
            bArr4[6] = -90;
            bArr4[7] = 4;
            bArr4[8] = -15;
            AbstractC0060n.x(bArr4, new byte[]{-10, 47, ((((~AbstractC0060n.class.getName().length()) | 766439966) & 2080506257) + ((AbstractC0060n.class.getName().length() & 1346374017) | 4507680)) ^ 2085014009, 112, -10, -112, -16, 84, -65});
            String intern = new String(bArr4, charset).intern();
            c0047j22.getClass();
            c(intern);
        } else {
            c4 = 7;
            c5 = '\b';
            c6 = 6;
            b4 = -123;
            c7 = 3;
            c8 = 5;
        }
        if (n4.a()) {
            c0047j22.getClass();
            byte[] bArr5 = new byte[9];
            bArr5[0] = -115;
            int i5 = ((~AbstractC0060n.class.getName().length()) | 1871945285) & 8783004;
            int length4 = (AbstractC0060n.class.getName().length() & 18090136) | 18352160;
            int i6 = ((i5 & length4) * 2) + (length4 ^ i5);
            bArr5[(i6 | 27135165) - (i6 & 27135165)] = 91;
            bArr5[2] = -52;
            bArr5[c7] = -23;
            bArr5[4] = 1;
            bArr5[c8] = 94;
            bArr5[c6] = 41;
            bArr5[c4] = 17;
            bArr5[c5] = b4;
            byte[] bArr6 = new byte[9];
            bArr6[0] = -2;
            bArr6[1] = 34;
            bArr6[2] = -65;
            int length5 = ((((~AbstractC0060n.class.getName().length()) | (-486890650)) & (-2130082268)) + ((AbstractC0060n.class.getName().length() & 18941056) | 270545104)) ^ (-1859537161);
            long j81 = -2145319819;
            long f4 = AbstractC0008a.f(AbstractC0060n.class, -1) | 519000031;
            long j82 = ((((((((j81 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((j81 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j81 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j81 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + ((((((((f4 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((f4 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((f4 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((f4 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845);
            long j83 = (j82 >>> 48) & 43690;
            long j84 = ((j83 >>> 2) | (j83 >>> 1)) & 858993459;
            long j85 = (j84 | (j84 >>> 2)) & 252645135;
            long j86 = (j82 >>> 32) & 43690;
            long j87 = ((j86 >>> 2) | (j86 >>> 1)) & 858993459;
            long j88 = ((j87 >>> 2) | j87) & 252645135;
            long j89 = (((j85 | (j85 >>> 4)) & 16711935) << 24) | ((((j88 >>> 4) | j88) & 16711935) << 16);
            long j90 = (j82 >>> 16) & 43690;
            long j91 = ((j90 >>> 2) | (j90 >>> 1)) & 858993459;
            long j92 = ((j91 >>> 2) | j91) & 252645135;
            long j93 = j82 & 43690;
            long j94 = ((j93 >>> 2) | (j93 >>> 1)) & 858993459;
            long j95 = (j94 | (j94 >>> 2)) & 252645135;
            int i7 = (int) (((j95 | (j95 >>> 4)) & 16711935) + (j89 | ((((j92 >>> 4) | j92) & 16711935) << c5)));
            long j96 = 1108222208;
            long length6 = AbstractC0060n.class.getName().length() & (-2147344352);
            long b5 = c3.b((((((((j96 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48, ((((((((j96 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j96 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j96 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845), ((((((((length6 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((length6 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((length6 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length6 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)), 6148914691236517205L);
            long j97 = (b5 >>> 48) & 43690;
            long j98 = ((j97 >>> 2) | (j97 >>> 1)) & 858993459;
            long j99 = (j98 | (j98 >>> 2)) & 252645135;
            long j100 = (b5 >>> 32) & 43690;
            long j101 = ((j100 >>> 2) | (j100 >>> 1)) & 858993459;
            long j102 = ((j101 >>> 2) | j101) & 252645135;
            long j103 = (((j99 | (j99 >>> 4)) & 16711935) << 24) | ((((j102 >>> 4) | j102) & 16711935) << 16);
            long j104 = (b5 >>> 16) & 43690;
            long j105 = ((j104 >>> 2) | (j104 >>> 1)) & 858993459;
            long j106 = ((j105 >>> 2) | j105) & 252645135;
            long j107 = b5 & 43690;
            long j108 = ((j107 >>> 2) | (j107 >>> 1)) & 858993459;
            long j109 = (j108 | (j108 >>> 2)) & 252645135;
            bArr6[length5] = (i7 + ((int) (((j109 | (j109 >>> 4)) & 16711935) + (j103 | ((((j106 >>> 4) | j106) & 16711935) << c5))))) ^ 1037097704;
            bArr6[4] = 100;
            int i8 = ((~AbstractC0060n.class.getName().length()) | (-696540939)) & (-2045706173);
            long j110 = 1342236692;
            long length7 = AbstractC0060n.class.getName().length() & 268451842;
            long j111 = (((((((((j110 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j110 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j110 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j110 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)))) + ((((((((length7 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((length7 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((length7 >>> c5) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((length7 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + 6148914691236517205L;
            long j112 = (j111 >>> 48) & 43690;
            long j113 = ((j112 >>> 2) | (j112 >>> 1)) & 858993459;
            long j114 = (j113 | (j113 >>> 2)) & 252645135;
            long j115 = (j111 >>> 32) & 43690;
            long j116 = ((j115 >>> 2) | (j115 >>> 1)) & 858993459;
            long j117 = (j116 | (j116 >>> 2)) & 252645135;
            long j118 = (((j114 | (j114 >>> 4)) & 16711935) << 24) | (((j117 | (j117 >>> 4)) & 16711935) << 16);
            long j119 = (j111 >>> 16) & 43690;
            long j120 = ((j119 >>> 2) | (j119 >>> 1)) & 858993459;
            long j121 = (j120 | (j120 >>> 2)) & 252645135;
            long j122 = j111 & 43690;
            long j123 = ((j122 >>> 2) | (j122 >>> 1)) & 858993459;
            long j124 = (j123 | (j123 >>> 2)) & 252645135;
            bArr6[c8] = (-703469468) ^ (i8 + ((int) (((j124 | (j124 >>> 4)) & 16711935) + ((((j121 | (j121 >>> 4)) & 16711935) << c5) + j118))));
            bArr6[c6] = Byte.MAX_VALUE;
            bArr6[c4] = 65;
            bArr6[c5] = -53;
            AbstractC0060n.x(bArr5, bArr6);
            c0054l1.c(new String(bArr5, charset).intern(), 1);
        }
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:3:0x005d. Please report as an issue. */
    public final boolean C(Context context) {
        ConnectivityManager connectivityManager;
        Exception exc;
        Network network;
        ConnectivityManager connectivityManager2;
        Charset charset;
        String intern;
        byte[] bArr;
        char c4 = 2218;
        ConnectivityManager connectivityManager3 = null;
        Exception exc2 = null;
        Network network2 = null;
        ConnectivityManager connectivityManager4 = null;
        ConnectivityManager connectivityManager5 = null;
        Network network3 = null;
        Object obj = null;
        boolean z3 = false;
        boolean z4 = false;
        while (true) {
            switch (c4) {
                case 34851:
                    ConnectivityManager connectivityManager6 = connectivityManager3;
                    Exception exc3 = exc2;
                    ConnectivityManager connectivityManager7 = connectivityManager4;
                    network2 = connectivityManager6.getActiveNetwork();
                    c4 = network2 == null ? (char) 13505 : (char) 49440;
                    connectivityManager3 = connectivityManager6;
                    connectivityManager5 = connectivityManager3;
                    exc2 = exc3;
                    connectivityManager4 = connectivityManager7;
                case 32196:
                    ConnectivityManager connectivityManager8 = connectivityManager3;
                    Exception exc4 = exc2;
                    Network network4 = network2;
                    long j2 = 557156696;
                    long j3 = (~C0015b2.class.getName().length()) | 435049232;
                    long j4 = (((((((((j2 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j2 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j2 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j2 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)))) + (((((((((j3 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j3 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j3 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j3 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845));
                    long j5 = (j4 >>> 48) & 43690;
                    long j6 = ((j5 >>> 2) | (j5 >>> 1)) & 858993459;
                    long j7 = ((j6 >>> 2) | j6) & 252645135;
                    long j8 = (j4 >>> 32) & 43690;
                    long j9 = ((j8 >>> 2) | (j8 >>> 1)) & 858993459;
                    long j10 = ((j9 >>> 2) | j9) & 252645135;
                    long j11 = ((((j10 >>> 4) | j10) & 16711935) << 16) | ((((j7 >>> 4) | j7) & 16711935) << 24);
                    long j12 = (j4 >>> 16) & 43690;
                    long j13 = ((j12 >>> 2) | (j12 >>> 1)) & 858993459;
                    long j14 = ((j13 >>> 2) | j13) & 252645135;
                    long j15 = j4 & 43690;
                    long j16 = ((j15 >>> 2) | (j15 >>> 1)) & 858993459;
                    long j17 = (j16 | (j16 >>> 2)) & 252645135;
                    int i = (int) (((j17 | (j17 >>> 4)) & 16711935) | (((((j14 >>> 4) | j14) & 16711935) << 8) + j11));
                    int length = (C0015b2.class.getName().length() & (-1609455416)) | (-2139070336);
                    byte[] bArr2 = {19, -49, -91, -12, -85, 119, -10, -19, -95, 1581913651 ^ (((length | i) * 2) - (i ^ length)), -110};
                    x(bArr2, new byte[]{69, -65, -53, -80, -50, 3, -109, -114, -43, -124, -32});
                    Charset charset2 = StandardCharsets.UTF_8;
                    new String(bArr2, charset2).intern();
                    byte[] bArr3 = new byte[26];
                    bArr3[0] = 33;
                    bArr3[1] = 74;
                    bArr3[2] = 73;
                    bArr3[3] = 40;
                    bArr3[((((~C0015b2.class.getName().length()) | (-461733026)) & 1074442505) + ((C0015b2.class.getName().length() & 67121169) | (-1945829356))) ^ (-871386855)] = 93;
                    bArr3[5] = 123;
                    bArr3[6] = -34;
                    bArr3[7] = -124;
                    bArr3[8] = 115;
                    bArr3[9] = -123;
                    bArr3[10] = 123;
                    bArr3[11] = 44;
                    bArr3[12] = 53;
                    bArr3[13] = -56;
                    bArr3[14] = 72;
                    int length2 = (((r4 | (-852928864)) - 2010627416) - ((~C0015b2.class.getName().length()) | (-852928856))) + ((C0015b2.class.getName().length() & 536907784) | 1879543808);
                    bArr3[15] = (length2 + 131083546) - ((length2 & 131083546) * 2);
                    bArr3[16] = -101;
                    int i3 = ~C0015b2.class.getName().length();
                    int i4 = ((~i3) & (-254058588)) + i3;
                    int i5 = (1460176744 | i4) - (i4 ^ 1460176744);
                    ConnectivityManager connectivityManager9 = connectivityManager4;
                    long j18 = 119832650;
                    long length3 = C0015b2.class.getName().length();
                    long j19 = (((((((((j18 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j18 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j18 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j18 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + (((((((((length3 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((length3 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((length3 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length3 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
                    long j20 = (j19 >>> 48) & 43690;
                    long j21 = ((j20 >>> 2) | (j20 >>> 1)) & 858993459;
                    long j22 = ((j21 >>> 2) | j21) & 252645135;
                    long j23 = (j19 >>> 32) & 43690;
                    long j24 = ((j23 >>> 2) | (j23 >>> 1)) & 858993459;
                    long j25 = ((j24 >>> 2) | j24) & 252645135;
                    long j26 = ((((j25 >>> 4) | j25) & 16711935) << 16) | ((((j22 >>> 4) | j22) & 16711935) << 24);
                    long j27 = (j19 >>> 16) & 43690;
                    long j28 = ((j27 >>> 2) | (j27 >>> 1)) & 858993459;
                    long j29 = ((j28 >>> 2) | j28) & 252645135;
                    long j30 = j19 & 43690;
                    long j31 = ((j30 >>> 2) | (j30 >>> 1)) & 858993459;
                    long j32 = (j31 | (j31 >>> 2)) & 252645135;
                    bArr3[17] = (i5 + (((int) (((j32 | (j32 >>> 4)) & 16711935) + (((((j29 >>> 4) | j29) & 16711935) << 8) + j26))) | (-2140862334))) ^ (-680685686);
                    bArr3[18] = -91;
                    bArr3[19] = 18;
                    bArr3[20] = -32;
                    bArr3[21] = -15;
                    int i6 = ((~C0015b2.class.getName().length()) | (-1378409161)) & 239084596;
                    int length4 = C0015b2.class.getName().length();
                    bArr3[22] = 1061385428 ^ ((((587206656 & length4) + 822300800) - (length4 & 553652224)) + i6);
                    bArr3[23] = -59;
                    bArr3[24] = -109;
                    long j33 = 9668945;
                    long j34 = (~C0015b2.class.getName().length()) | (-538131915);
                    long j35 = (((((((((j33 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j33 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j33 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j33 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)))) + (((((((((j34 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j34 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((j34 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j34 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
                    long j36 = (j35 >>> 48) & 43690;
                    long j37 = ((j36 >>> 2) | (j36 >>> 1)) & 858993459;
                    long j38 = ((j37 >>> 2) | j37) & 252645135;
                    long j39 = (j35 >>> 32) & 43690;
                    long j40 = ((j39 >>> 2) | (j39 >>> 1)) & 858993459;
                    long j41 = ((j40 >>> 2) | j40) & 252645135;
                    long j42 = ((((j41 >>> 4) | j41) & 16711935) << 16) | ((((j38 >>> 4) | j38) & 16711935) << 24);
                    long j43 = (j35 >>> 16) & 43690;
                    long j44 = ((j43 >>> 2) | (j43 >>> 1)) & 858993459;
                    long j45 = ((j44 >>> 2) | j44) & 252645135;
                    long j46 = j35 & 43690;
                    long j47 = ((j46 >>> 2) | (j46 >>> 1)) & 858993459;
                    long j48 = ((j47 >>> 2) | j47) & 252645135;
                    bArr3[25] = (((int) ((((j48 >>> 4) | j48) & 16711935) + (((((j45 >>> 4) | j45) & 16711935) << 8) | j42))) + ((C0015b2.class.getName().length() & 135473600) | 151003776)) ^ (-160672758);
                    byte[] bArr4 = new byte[26];
                    bArr4[0] = 70;
                    bArr4[1] = 47;
                    bArr4[2] = 61;
                    bArr4[3] = 102;
                    bArr4[4] = 56;
                    bArr4[5] = 15;
                    bArr4[6] = -87;
                    bArr4[7] = -21;
                    bArr4[((((~C0015b2.class.getName().length()) | (-2)) + 101582338) + ((C0015b2.class.getName().length() & 49153) | (-1995390974))) ^ (-1893808629)] = 1;
                    bArr4[9] = -18;
                    bArr4[10] = 56;
                    bArr4[11] = 77;
                    bArr4[12] = 69;
                    long j49 = -1216829462;
                    long a2 = I2.a(C0015b2.class, -1);
                    long b4 = c3.b((((((((j49 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48, ((((((((j49 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((j49 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j49 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845), ((((((((a2 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((a2 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((a2 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((a2 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)), 6148914691236517205L);
                    long j50 = (b4 >>> 48) & 43690;
                    long j51 = ((j50 >>> 2) | (j50 >>> 1)) & 858993459;
                    long j52 = ((j51 >>> 2) | j51) & 252645135;
                    long j53 = (b4 >>> 32) & 43690;
                    long j54 = ((j53 >>> 2) | (j53 >>> 1)) & 858993459;
                    long j55 = ((j54 >>> 2) | j54) & 252645135;
                    long j56 = ((((j55 >>> 4) | j55) & 16711935) << 16) + ((((j52 >>> 4) | j52) & 16711935) << 24);
                    long j57 = (b4 >>> 16) & 43690;
                    long j58 = ((j57 >>> 2) | (j57 >>> 1)) & 858993459;
                    long j59 = ((j58 >>> 2) | j58) & 252645135;
                    long j60 = b4 & 43690;
                    long j61 = ((j60 >>> 2) | (j60 >>> 1)) & 858993459;
                    long j62 = ((j61 >>> 2) | j61) & 252645135;
                    bArr4[13] = ((((int) ((((j62 >>> 4) | j62) & 16711935) | (((((j59 >>> 4) | j59) & 16711935) << 8) + j56))) & 336085769) + ((C0015b2.class.getName().length() & 12599345) | 13632560)) ^ (-349718384);
                    bArr4[14] = 42;
                    bArr4[15] = -37;
                    bArr4[16] = -9;
                    bArr4[17] = 9;
                    bArr4[18] = -47;
                    bArr4[19] = 123;
                    bArr4[20] = -123;
                    bArr4[21] = -126;
                    bArr4[22] = 72;
                    bArr4[23] = -20;
                    bArr4[24] = -87;
                    long j63 = -2052262223;
                    long length5 = (((~C0015b2.class.getName().length()) | (-394814583)) & (-2128558424)) + ((C0015b2.class.getName().length() & 25457696) | 76296192);
                    long j64 = ((((((((j63 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + (((((((((j63 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j63 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j63 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)) + ((((((((length5 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((length5 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((length5 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length5 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845);
                    long j65 = (j64 >>> 48) & 21845;
                    long j66 = (j65 | (j65 >>> 1)) & 858993459;
                    long j67 = (j66 | (j66 >>> 2)) & 252645135;
                    long j68 = (j64 >>> 32) & 21845;
                    long j69 = ((j68 >>> 1) | j68) & 858993459;
                    long j70 = ((j69 >>> 2) | j69) & 252645135;
                    long j71 = (((j67 | (j67 >>> 4)) & 16711935) << 24) | ((((j70 >>> 4) | j70) & 16711935) << 16);
                    long j72 = (j64 >>> 16) & 21845;
                    long j73 = ((j72 >>> 1) | j72) & 858993459;
                    long j74 = ((j73 >>> 2) | j73) & 252645135;
                    long j75 = j64 & 21845;
                    long j76 = (j75 | (j75 >>> 1)) & 858993459;
                    long j77 = (j76 | (j76 >>> 2)) & 252645135;
                    bArr4[(int) (j71 | ((((j74 >>> 4) | j74) & 16711935) << 8) | ((j77 | (j77 >>> 4)) & 16711935))] = -5;
                    x(bArr3, bArr4);
                    new String(bArr3, charset2).intern();
                    Objects.toString(exc4);
                    network2 = network4;
                    z3 = false;
                    connectivityManager3 = connectivityManager8;
                    exc2 = exc4;
                    connectivityManager4 = connectivityManager9;
                    c4 = 56496;
                case 52855:
                    c4 = 56496;
                case 42808:
                case 49286:
                    c4 = 10581;
                    z4 = false;
                case 49440:
                    c4 = 19027;
                    network3 = network2;
                case 2218:
                    connectivityManager = connectivityManager3;
                    byte[] bArr5 = {108, 68, -2, 121, -25, -27, 75, 99, 98, -27, -56, 59};
                    byte[] bArr6 = new byte[12];
                    int i7 = ((~C0015b2.class.getName().length()) | (-1912157138)) & (-933863167);
                    exc = exc2;
                    network = network2;
                    long j78 = 603979938;
                    long length6 = C0015b2.class.getName().length() & 1615864225;
                    long b5 = c3.b((((((((j78 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48, ((((((((j78 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j78 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j78 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845), ((((((((length6 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + ((((((((length6 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((length6 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((length6 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)), 6148914691236517205L);
                    long j79 = (b5 >>> 48) & 43690;
                    long j80 = ((j79 >>> 2) | (j79 >>> 1)) & 858993459;
                    long j81 = ((j80 >>> 2) | j80) & 252645135;
                    long j82 = (b5 >>> 32) & 43690;
                    long j83 = ((j82 >>> 2) | (j82 >>> 1)) & 858993459;
                    long j84 = ((j83 >>> 2) | j83) & 252645135;
                    long j85 = ((((j84 >>> 4) | j84) & 16711935) << 16) | ((((j81 >>> 4) | j81) & 16711935) << 24);
                    long j86 = (b5 >>> 16) & 43690;
                    long j87 = ((j86 >>> 2) | (j86 >>> 1)) & 858993459;
                    long j88 = ((j87 >>> 2) | j87) & 252645135;
                    long j89 = b5 & 43690;
                    long j90 = ((j89 >>> 2) | (j89 >>> 1)) & 858993459;
                    long j91 = (j90 | (j90 >>> 2)) & 252645135;
                    bArr6[0] = (-329883220) ^ (i7 + ((int) (((j91 | (j91 >>> 4)) & 16711935) + (((((j88 >>> 4) | j88) & 16711935) << 8) + j85))));
                    bArr6[1] = 43;
                    bArr6[2] = -112;
                    bArr6[3] = 23;
                    bArr6[4] = -126;
                    bArr6[5] = -122;
                    bArr6[6] = 63;
                    long j92 = -1;
                    long length7 = C0015b2.class.getName().length();
                    long j93 = (((((((((j92 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j92 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + (((((((((j92 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j92 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)))) + ((((((((length7 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + (((((((((length7 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((length7 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length7 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
                    long j94 = (j93 >>> 48) & 21845;
                    long j95 = (j94 | (j94 >>> 1)) & 858993459;
                    long j96 = (j95 | (j95 >>> 2)) & 252645135;
                    long j97 = (j93 >>> 32) & 21845;
                    long j98 = ((j97 >>> 1) | j97) & 858993459;
                    long j99 = ((j98 >>> 2) | j98) & 252645135;
                    long j100 = ((((j99 >>> 4) | j99) & 16711935) << 16) + (((j96 | (j96 >>> 4)) & 16711935) << 24);
                    long j101 = (j93 >>> 16) & 21845;
                    long j102 = ((j101 >>> 1) | j101) & 858993459;
                    long j103 = ((j102 >>> 2) | j102) & 252645135;
                    long j104 = j93 & 21845;
                    long j105 = (j104 | (j104 >>> 1)) & 858993459;
                    long j106 = (j105 | (j105 >>> 2)) & 252645135;
                    bArr6[(((((int) ((((((j103 >>> 4) | j103) & 16711935) << 8) + j100) | ((j106 | (j106 >>> 4)) & 16711935))) | 2143289343) - 1421320063) + ((C0015b2.class.getName().length() & (-2143289337)) | 1141182503)) ^ (-280137568)] = ((((~C0015b2.class.getName().length()) | (-1225012839)) & 1115816457) + ((C0015b2.class.getName().length() & 1074012800) | 287872)) ^ 1116104323;
                    bArr6[8] = 20;
                    bArr6[9] = -116;
                    bArr6[10] = -68;
                    bArr6[11] = 66;
                    x(bArr5, bArr6);
                    obj = context.getSystemService(new String(bArr5, StandardCharsets.UTF_8).intern());
                    if (obj instanceof ConnectivityManager) {
                        c4 = 37318;
                        network2 = network;
                        connectivityManager3 = connectivityManager;
                        exc2 = exc;
                    }
                    c4 = 37427;
                    network2 = network;
                    connectivityManager3 = connectivityManager;
                    exc2 = exc;
                case 34914:
                    c4 = connectivityManager4 == null ? (char) 54015 : (char) 34851;
                    connectivityManager3 = connectivityManager4;
                case 13505:
                case 54015:
                    return false;
                case 31448:
                    try {
                        byte[] bArr7 = {-50, -62, -83, -22, -6, -88, -31, 111, 95, -87, 102, 53};
                        int length8 = (((~C0015b2.class.getName().length()) | (-380205173)) & 1095516164) + (((C0015b2.class.getName().length() | (-68698119)) + 68698119) | 68288522);
                        byte a4 = E1.a(length8 | (-1163804780), -1163804780, length8);
                        connectivityManager2 = connectivityManager3;
                        long j107 = 654553153;
                        long j108 = (~C0015b2.class.getName().length()) | 443137043;
                        long j109 = ((((((((j107 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) + (((((((((j107 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | (((((((((j107 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j107 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))) + (((((((((j108 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | ((((((((j108 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) | ((((((((j108 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) | ((((((j108 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845));
                        long j110 = (j109 >>> 48) & 43690;
                        long j111 = ((j110 >>> 2) | (j110 >>> 1)) & 858993459;
                        long j112 = ((j111 >>> 2) | j111) & 252645135;
                        long j113 = (j109 >>> 32) & 43690;
                        long j114 = ((j113 >>> 2) | (j113 >>> 1)) & 858993459;
                        long j115 = ((j114 >>> 2) | j114) & 252645135;
                        long j116 = ((((j115 >>> 4) | j115) & 16711935) << 16) | ((((j112 >>> 4) | j112) & 16711935) << 24);
                        long j117 = (j109 >>> 16) & 43690;
                        long j118 = ((j117 >>> 2) | (j117 >>> 1)) & 858993459;
                        long j119 = ((j118 >>> 2) | j118) & 252645135;
                        long j120 = j109 & 43690;
                        long j121 = ((j120 >>> 2) | (j120 >>> 1)) & 858993459;
                        long j122 = (j121 | (j121 >>> 2)) & 252645135;
                        try {
                            byte length9 = (((int) (((j122 | (j122 >>> 4)) & 16711935) + (((((j119 >>> 4) | j119) & 16711935) << 8) + j116))) + ((C0015b2.class.getName().length() & (-1526595392)) | (-2143551352))) ^ 1488998195;
                            long j123 = -1;
                            long length10 = C0015b2.class.getName().length();
                            long j124 = (((((((((j123 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((j123 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((j123 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j123 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845))) + (((((((((length10 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((length10 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((length10 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length10 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)));
                            long j125 = (j124 >>> 48) & 21845;
                            long j126 = ((j125 >>> 1) | j125) & 858993459;
                            long j127 = ((j126 >>> 2) | j126) & 252645135;
                            long j128 = (j124 >>> 32) & 21845;
                            long j129 = ((j128 >>> 1) | j128) & 858993459;
                            long j130 = ((j129 >>> 2) | j129) & 252645135;
                            long j131 = ((((j130 >>> 4) | j130) & 16711935) << 16) | ((((j127 >>> 4) | j127) & 16711935) << 24);
                            long j132 = (j124 >>> 16) & 21845;
                            long j133 = ((j132 >>> 1) | j132) & 858993459;
                            long j134 = ((j133 >>> 2) | j133) & 252645135;
                            long j135 = ((((j134 >>> 4) | j134) & 16711935) << 8) + j131;
                            long j136 = j124 & 21845;
                            long j137 = (j136 | (j136 >>> 1)) & 858993459;
                            long j138 = (j137 | (j137 >>> 2)) & 252645135;
                            int i8 = (((int) (((j138 | (j138 >>> 4)) & 16711935) + j135)) | 332250361) & 37824100;
                            long j139 = 135446808;
                            long length11 = C0015b2.class.getName().length() & 164364;
                            long b6 = c3.b((((((((j139 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48, ((((((((j139 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((j139 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((j139 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845), ((((((((length11 >>> 24) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 48) | (((((((((length11 >>> 16) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 32) + ((((((((length11 >>> 8) & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845) << 16) + ((((((length11 & 255) * 72340172838076673L) & (-9205322385119247871L)) * 72624976668147841L) >>> 49) & 21845)), 6148914691236517205L);
                            long j140 = (b6 >>> 48) & 43690;
                            long j141 = ((j140 >>> 2) | (j140 >>> 1)) & 858993459;
                            long j142 = ((j141 >>> 2) | j141) & 252645135;
                            long j143 = (b6 >>> 32) & 43690;
                            long j144 = ((j143 >>> 2) | (j143 >>> 1)) & 858993459;
                            long j145 = ((j144 >>> 2) | j144) & 252645135;
                            long j146 = ((((j145 >>> 4) | j145) & 16711935) << 16) + ((((j142 >>> 4) | j142) & 16711935) << 24);
                            long j147 = (b6 >>> 16) & 43690;
                            long j148 = ((j147 >>> 2) | (j147 >>> 1)) & 858993459;
                            long j149 = ((j148 >>> 2) | j148) & 252645135;
                            long j150 = b6 & 43690;
                            long j151 = ((j150 >>> 2) | (j150 >>> 1)) & 858993459;
                            long j152 = ((j151 >>> 2) | j151) & 252645135;
                            byte b7 = (i8 + ((int) ((((j152 >>> 4) | j152) & 16711935) + (((((j149 >>> 4) | j149) & 16711935) << 8) + j146)))) ^ (-173270808);
                            byte length12 = ((((~C0015b2.class.getName().length()) | 1738805891) & (-233822702)) + ((C0015b2.class.getName().length() & (-1873763280)) | 4229216)) ^ 229593522;
                            int i9 = ((~C0015b2.class.getName().length()) | (-524293)) - (-187172109);
                            int length13 = (C0015b2.class.getName().length() & 67665956) | 75612832;
                            int i10 = -i9;
                            x(bArr7, new byte[]{-89, -79, -5, a4, -108, length9, b7, 1, 49, length12, 262784932 ^ (((~i10) & length13) - (i10 & (~length13))), 82});
                            charset = StandardCharsets.UTF_8;
                            intern = new String(bArr7, charset).intern();
                            bArr = new byte[]{-29, -36, -54, 39};
                            x(bArr, new byte[]{-105, -82, -65, 66, -54, -105, -107, -52});
                        } catch (Exception e4) {
                            e = e4;
                        }
                    } catch (Exception e5) {
                        e = e5;
                        connectivityManager2 = connectivityManager3;
                    }
                    try {
                        t(intern, new String(bArr, charset).intern());
                        c4 = 10581;
                        z4 = true;
                    } catch (Exception e6) {
                        e = e6;
                        exc2 = e;
                        c4 = 32196;
                        connectivityManager3 = connectivityManager2;
                    }
                    connectivityManager3 = connectivityManager2;
                case 19027:
                    try {
                        obj = connectivityManager5.getNetworkCapabilities(network3);
                        c4 = obj != null ? (char) 17664 : (char) 42808;
                    } catch (Exception e7) {
                        exc2 = e7;
                        c4 = 32196;
                    }
                case 17664:
                    try {
                        c4 = ((NetworkCapabilities) obj).hasTransport(4) ? (char) 31448 : (char) 49286;
                    } catch (Exception e8) {
                        exc2 = e8;
                        connectivityManager2 = connectivityManager3;
                        c4 = 32196;
                        connectivityManager3 = connectivityManager2;
                    }
                case 56496:
                    return z3;
                case 37318:
                    connectivityManager4 = (ConnectivityManager) obj;
                    c4 = 34914;
                case 10581:
                    c4 = 52855;
                    z3 = z4;
                case 37427:
                    c4 = 34914;
                    connectivityManager4 = null;
                default:
                    connectivityManager = connectivityManager3;
                    exc = exc2;
                    network = network2;
                    c4 = 37427;
                    network2 = network;
                    connectivityManager3 = connectivityManager;
                    exc2 = exc;
            }
        }
    }

    /*  JADX ERROR: NullPointerException in pass: InitCodeVariables
        java.lang.NullPointerException: Cannot invoke "jadx.core.dex.instructions.args.SSAVar.getPhiList()" because "resultVar" is null
        	at jadx.core.dex.visitors.InitCodeVariables.collectConnectedVars(InitCodeVariables.java:119)
        	at jadx.core.dex.visitors.InitCodeVariables.setCodeVar(InitCodeVariables.java:82)
        	at jadx.core.dex.visitors.InitCodeVariables.initCodeVar(InitCodeVariables.java:74)
        	at jadx.core.dex.visitors.InitCodeVariables.initCodeVars(InitCodeVariables.java:48)
        	at jadx.core.dex.visitors.InitCodeVariables.visit(InitCodeVariables.java:29)
        */
    @Override // F0.T2
    public final void a(android.content.Context r37) {
        /*
            Method dump skipped, instructions count: 1090
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: F0.C0015b2.a(android.content.Context):void");
    }
}
