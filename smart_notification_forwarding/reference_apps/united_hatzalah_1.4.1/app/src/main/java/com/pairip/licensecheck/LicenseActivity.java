...
public class LicenseActivity extends Activity {
    ...
    public void onStart() {
        ...
        try {
            ...
            logAndShowErrorDialog("Couldn't process license activity correctly.", e4);
        }
    }
    ...
    public /* synthetic */ void lambda$showPaywallAndCloseApp$0(PendingIntent pendingIntent) {
        try {
            ...
            logAndShowErrorDialog("Paywall intent unexpectedly cancelled.", e4);
        }
    }

    private void logAndShowErrorDialog(String message, Exception ex) {
        logAndShowErrorDialog(message + StringUtils.SPACE + Log.getStackTraceString(ex));
    }
    ...
    public /* synthetic */ void lambda$showErrorDialog$0() {
        try {
            ...
            Log.d(TAG, "Couldn't show the error dialog. " + Log.getStackTraceString(e4));
        }
    }
        ...
        if (activityManager != null) {
            for (ActivityManager.AppTask appTask : activityManager.getAppTasks()) {
                try {
                    ...
                    Log.e(TAG, "Failed to gracefully clear task=" + appTask.getTaskInfo().id, e4);
                }
            }
        }
        exitApp();
