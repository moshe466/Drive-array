...
public class Validation {
    ...
    private static boolean isWritableKey(String str) {
        if (str != null && str.length() > 0) {
            if (!str.equals(".value") && !str.equals(".priority")) {
                if (!str.startsWith(".") && !INVALID_KEY_REGEX.matcher(str).find()) {
                    return true;
                }
                return false;
            }
    ...
    public static void validateNullableKey(String str) {
        if (str != null && !isValidKey(str)) {
            throw new DatabaseException(a.e("Invalid key: ", str, ". Keys must not contain '/', '.', '#', '$', '[', or ']'"));
        }
    }
    ...
    public static void validatePathString(String str) {
        if (isValidPathString(str)) {
            ...
            throw new DatabaseException(a.e("Invalid Firebase Database path: ", str, ". Firebase Database paths must not contain '.', '#', '$', '[', or ']'"));
        }
    }
