...
public class URLCredentialDao {
    ...
    String[] projection = {"_id", URLCredentialContract.FeedEntry.COLUMN_NAME_USERNAME, "password", URLCredentialContract.FeedEntry.COLUMN_NAME_PROTECTION_SPACE_ID};
    ...
    public URLCredential find(String str, String str2, Long l3) {
        URLCredential uRLCredential;
        Cursor query = this.credentialDatabaseHelper.getReadableDatabase().query(URLCredentialContract.FeedEntry.TABLE_NAME, this.projection, "username = ? AND password = ? AND protection_space_id = ?", new String[]{str, str2, l3.toString()}, null, null, null);
        if (query.moveToNext()) {
            uRLCredential = new URLCredential(Long.valueOf(query.getLong(query.getColumnIndexOrThrow("_id"))), query.getString(query.getColumnIndexOrThrow(URLCredentialContract.FeedEntry.COLUMN_NAME_USERNAME)), query.getString(query.getColumnIndexOrThrow("password")), l3);
        } else {
            uRLCredential = null;
        }
        query.close();
    ...
    public List<URLCredential> getAllByProtectionSpaceId(Long l3) {
        Cursor query = this.credentialDatabaseHelper.getReadableDatabase().query(URLCredentialContract.FeedEntry.TABLE_NAME, this.projection, "protection_space_id = ?", new String[]{l3.toString()}, null, null, null);
        ArrayList arrayList = new ArrayList();
        while (query.moveToNext()) {
            arrayList.add(new URLCredential(Long.valueOf(query.getLong(query.getColumnIndexOrThrow("_id"))), query.getString(query.getColumnIndexOrThrow(URLCredentialContract.FeedEntry.COLUMN_NAME_USERNAME)), query.getString(query.getColumnIndexOrThrow("password")), l3));
        }
        query.close();
        return arrayList;
    }
