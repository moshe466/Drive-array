    ...
    public ActivityTransitionResult(List<ActivityTransitionEvent> list) {
        ...
        if (!list.isEmpty()) {
            for (int i = 1; i < list.size(); i++) {
                int i3 = i - 1;
                Preconditions.checkArgument(list.get(i).getElapsedRealTimeNanos() >= list.get(i3).getElapsedRealTimeNanos(), "Transition out of order: ts1=%d, ts2=%d", Long.valueOf(list.get(i).getElapsedRealTimeNanos()), Long.valueOf(list.get(i3).getElapsedRealTimeNanos()));
            }
        }
