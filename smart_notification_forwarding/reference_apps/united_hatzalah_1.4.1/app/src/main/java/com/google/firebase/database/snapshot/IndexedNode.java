...
public class IndexedNode implements Iterable<NamedNode> {
    private static final ImmutableSortedSet<NamedNode> FALLBACK_INDEX = new ImmutableSortedSet<>(Collections.EMPTY_LIST, null);
    ...
    private void ensureIndexed() {
        if (this.indexed == null) {
            ...
            for (NamedNode namedNode : this.node) {
                if (!z3 && !this.index.isDefinedOn(namedNode.getNode())) {
                    z3 = false;
                } else {
                    z3 = true;
                }
