...
public class DefaultPersistenceManager implements PersistenceManager {
    ...
    private void doPruneCheckAfterServerUpdate() {
        ...
        if (this.cachePolicy.shouldCheckCacheSize(j2)) {
            ...
            if (this.logger.logsDebug()) {
                this.logger.debug(AbstractC0008a.l("Cache size: ", serverCacheEstimatedSizeInBytes), new Object[0]);
            }
            boolean z3 = true;
            while (z3 && this.cachePolicy.shouldPrune(serverCacheEstimatedSizeInBytes, this.trackedQueryManager.countOfPrunableQueries())) {
                ...
                if (this.logger.logsDebug()) {
                    this.logger.debug(AbstractC0008a.l("Cache size after prune: ", serverCacheEstimatedSizeInBytes), new Object[0]);
                }
            }
        }
    }
    ...
    public CacheNode serverCache(QuerySpec querySpec) {
        ...
        if (this.trackedQueryManager.isQueryComplete(querySpec)) {
            ...
            if (!querySpec.loadsAllData() && findTrackedQuery != null && findTrackedQuery.complete) {
                knownCompleteChildren = this.storageLayer.loadTrackedQueryKeys(findTrackedQuery.id);
            } else {
        ...
        if (knownCompleteChildren != null) {
            ...
            return new CacheNode(IndexedNode.from(Empty, querySpec.getIndex()), z3, true);
        }
        return new CacheNode(IndexedNode.from(serverCache, querySpec.getIndex()), z3, false);
    }
    ...
    public void setTrackedQueryKeys(QuerySpec querySpec, Set<ChildKey> set) {
        ...
        Utilities.hardAssert(z3, "We only expect tracked keys for currently-active queries.");
