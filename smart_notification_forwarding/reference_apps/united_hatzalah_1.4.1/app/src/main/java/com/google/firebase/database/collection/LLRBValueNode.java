...
public abstract class LLRBValueNode<K, V> implements LLRBNode<K, V> {
    ...
    public LLRBValueNode(K k4, V v2, LLRBNode<K, V> lLRBNode, LLRBNode<K, V> lLRBNode2) {
        this.key = k4;
        this.value = v2;
    ...
    public abstract LLRBValueNode<K, V> copy(K k4, V v2, LLRBNode<K, V> lLRBNode, LLRBNode<K, V> lLRBNode2);
    ...
    @Override // com.google.firebase.database.collection.LLRBNode
    public LLRBNode<K, V> insert(K k4, V v2, Comparator<K> comparator) {
        LLRBValueNode<K, V> copy;
        int compare = comparator.compare(k4, this.key);
        if (compare < 0) {
            copy = copy(null, null, this.left.insert(k4, v2, comparator), null);
        } else if (compare == 0) {
            copy = copy(k4, v2, null, null);
        } else {
            copy = copy(null, null, null, this.right.insert(k4, v2, comparator));
        }
        return copy.fixUp();
    }
    ...
    public LLRBNode<K, V> remove(K k4, Comparator<K> comparator) {
        ...
        if (comparator.compare(k4, this.key) < 0) {
            ...
            copy = lLRBValueNode2.copy(null, null, lLRBValueNode2.left.remove(k4, comparator), null);
        } else {
            if (this.left.isRed()) {
            ...
            if (comparator.compare(k4, lLRBValueNode.key) == 0) {
            ...
            copy = lLRBValueNode.copy(null, null, null, lLRBValueNode.right.remove(k4, comparator));
        }
        return copy.fixUp();
    }
    ...
    @Override // com.google.firebase.database.collection.LLRBNode
    public LLRBValueNode<K, V> copy(K k4, V v2, LLRBNode.Color color, LLRBNode<K, V> lLRBNode, LLRBNode<K, V> lLRBNode2) {
        if (k4 == null) {
            k4 = this.key;
        }
        if (v2 == null) {
        ...
        return new LLRBBlackValueNode(k4, v2, lLRBNode, lLRBNode2);
    }
}
