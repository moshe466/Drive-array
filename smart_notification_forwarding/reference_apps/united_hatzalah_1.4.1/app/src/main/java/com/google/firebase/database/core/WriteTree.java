...
public class WriteTree {
    ...
    public void addMerge(Path path, CompoundWrite compoundWrite, Long l3) {
        boolean z3;
        if (l3.longValue() > this.lastWriteId.longValue()) {
            z3 = true;
        } else {
            z3 = false;
        }
        Utilities.hardAssert(z3);
        this.allWrites.add(new UserWriteRecord(l3.longValue(), path, compoundWrite));
        this.visibleWrites = this.visibleWrites.addWrites(path, compoundWrite);
        this.lastWriteId = l3;
    }

    public void addOverwrite(Path path, Node node, Long l3, boolean z3) {
        boolean z4;
        if (l3.longValue() > this.lastWriteId.longValue()) {
            z4 = true;
        } else {
        ...
        this.allWrites.add(new UserWriteRecord(l3.longValue(), path, node, z3));
        if (z3) {
    ...
    public Node calcEventCacheAfterServerOverwrite(Path path, Path path2, Node node, Node node2) {
        ...
        Utilities.hardAssert(z3, "Either existingEventSnap or existingServerSnap must exist");
    ...
    public NamedNode calcNextNodeAfterPost(Path path, Node node, NamedNode namedNode, boolean z3, Index index) {
        CompoundWrite childCompoundWrite = this.visibleWrites.childCompoundWrite(path);
        ...
        for (NamedNode namedNode3 : completeNode) {
            if (index.compare(namedNode3, namedNode, z3) > 0 && (namedNode2 == null || index.compare(namedNode3, namedNode2, z3) < 0)) {
    ...
    public UserWriteRecord getWrite(long j2) {
        for (UserWriteRecord userWriteRecord : this.allWrites) {
            if (userWriteRecord.getWriteId() == j2) {
    ...
    public boolean removeWrite(long j2) {
        ...
        Utilities.hardAssert(z3, "removeWrite called with nonexistent writeId");
    ...
    public Node calcCompleteEventCache(final Path path, Node node, final List<Long> list, final boolean z3) {
        if (list.isEmpty() && !z3) {
            Node completeNode = this.visibleWrites.getCompleteNode(path);
            if (completeNode != null) {
                return completeNode;
            ...
            if (z3 || !childCompoundWrite2.isEmpty()) {
                if (!z3 && node == null && !childCompoundWrite2.hasCompleteWrite(Path.getEmptyPath())) {
                    return null;
                }
                CompoundWrite layerTree = layerTree(this.allWrites, new Predicate<UserWriteRecord>() { // from class: com.google.firebase.database.core.WriteTree.1
                    @Override // com.google.firebase.database.core.utilities.Predicate
                    public boolean evaluate(UserWriteRecord userWriteRecord) {
                        if ((userWriteRecord.isVisible() || z3) && !list.contains(Long.valueOf(userWriteRecord.getWriteId()))) {
