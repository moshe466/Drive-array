...
public final class RandomKt {
    ...
    public static final long nextLong(@NotNull Random nextLong, @NotNull LongRange range) {
        ...
        if (!range.isEmpty()) {
            ...
            long longValue = range.getStart().longValue();
            return last < Long.MAX_VALUE ? nextLong.nextLong(longValue, range.getEndInclusive().longValue() + 1) : longValue > Long.MIN_VALUE ? nextLong.nextLong(range.getStart().longValue() - 1, range.getEndInclusive().longValue()) + 1 : nextLong.nextLong();
        }
        throw new IllegalArgumentException("Cannot get random in empty range: " + range);
    }
