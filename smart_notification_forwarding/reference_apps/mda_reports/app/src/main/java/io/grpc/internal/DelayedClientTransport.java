...
public final class DelayedClientTransport implements ManagedClientTransport {
        ...
        synchronized (this.lock) {
            ...
            if (subchannelPicker != null && hasPendingStreams()) {
                ...
                while (it.hasNext()) {
                    ...
                    final ClientTransport a = GrpcUtil.a(pickSubchannel, callOptions.isWaitForReady());
                    if (a != null) {
                        ...
                        executor.execute(new Runnable(this) { // from class: io.grpc.internal.DelayedClientTransport.5
                            ...
                            public void run() {
                                pendingStream.createRealStream(a);
                            }
                        });
    ...
    public final ClientStream newStream(MethodDescriptor<?, ?> methodDescriptor, Metadata metadata, CallOptions callOptions) {
        ...
        try {
            ...
            do {
                synchronized (this.lock) {
                    if (this.shutdownStatus != null) {
                        ...
                        a = GrpcUtil.a(subchannelPicker.pickSubchannel(pickSubchannelArgsImpl), callOptions.isWaitForReady());
                    } else {
                        failingClientStream = createPendingStream(pickSubchannelArgsImpl);
                    }
                    break;
            ...
            failingClientStream = a.newStream(pickSubchannelArgsImpl.getMethodDescriptor(), pickSubchannelArgsImpl.getHeaders(), pickSubchannelArgsImpl.getCallOptions());
