...
public final class zzbeo {
    private static final Pattern zzehq = Pattern.compile("^\\uFEFF?\\s*(\\s*<!--([^-]|(?!-->))*-->)*\\s*<!DOCTYPE(\\s)+html(|(\\s)+[^>]*)>", 2);
    private static final Pattern zzehr = Pattern.compile("^\\uFEFF?\\s*(\\s*<!--([^-]|(?!-->))*-->)*?\\s*<!DOCTYPE[^>]*>", 2);
    ...
    public static String zzabp() {
        ...
        try {
            ...
            zzayu.zzd("Unable to build MRAID_ENV", e);
            return null;
        }
    }
    ...
    public static String zzf(@NonNull String str, @Nullable String... strArr) {
        ...
        Matcher matcher = zzehq.matcher(str);
        int i = 0;
        if (matcher.find()) {
            int end = matcher.end();
        ...
        } else if (!zzehr.matcher(str).find()) {
