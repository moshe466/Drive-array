...
final class TransportImpl<T> implements Transport<T> {
    ...
    public void schedule(Event<T> event, TransportScheduleCallback transportScheduleCallback) {
        this.transportInternal.send(SendRequest.builder().setTransportContext(this.transportContext).a((Event<?>) event).setTransportName(this.name).a((Transformer<?, byte[]>) this.transformer).a(this.payloadEncoding).build(), transportScheduleCallback);
    }
