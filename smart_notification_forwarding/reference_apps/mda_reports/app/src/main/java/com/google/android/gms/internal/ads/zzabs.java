...
public final class zzabs extends RelativeLayout {
    ...
    public zzabs(Context context, zzabp zzabpVar, RelativeLayout.LayoutParams layoutParams) {
        ...
        if (zzqy != null && zzqy.size() > 1) {
            ...
            Iterator<zzabu> it = zzqy.iterator();
            while (it.hasNext()) {
                try {
                    this.zzcvl.addFrame((Drawable) ObjectWrapper.unwrap(it.next().zzrc()), zzabpVar.zzqz());
                } catch (Exception e) {
                    zzayu.zzc("Error while getting drawable.", e);
                }
            }
            com.google.android.gms.ads.internal.zzq.zzks();
            ...
            try {
                ...
                zzayu.zzc("Error while getting drawable.", e2);
            }
        }
        addView(imageView);
    }
