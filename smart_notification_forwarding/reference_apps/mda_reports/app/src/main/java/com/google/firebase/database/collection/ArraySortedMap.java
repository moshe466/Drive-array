...
public class ArraySortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    private Iterator<Map.Entry<K, V>> iterator(final int i, final boolean z) {
        return new Iterator<Map.Entry<K, V>>() { // from class: com.google.firebase.database.collection.ArraySortedMap.1
            ...
            public boolean hasNext() {
                ...
                } else if (this.a < ArraySortedMap.this.keys.length) {
                    return true;
                }
                return false;
            }
            ...
            public Map.Entry<K, V> next() {
                Object obj = ArraySortedMap.this.keys[this.a];
    ...
    public ImmutableSortedMap<K, V> insert(K k, V v) {
        int findKey = findKey(k);
        if (findKey != -1) {
            if (this.keys[findKey] == k && this.values[findKey] == v) {
                return this;
            }
    ...
    public Iterator<Map.Entry<K, V>> reverseIteratorFrom(K k) {
        int findKeyOrInsertPosition = findKeyOrInsertPosition(k);
        K[] kArr = this.keys;
        return (findKeyOrInsertPosition >= kArr.length || this.comparator.compare(kArr[findKeyOrInsertPosition], k) != 0) ? iterator(findKeyOrInsertPosition - 1, true) : iterator(findKeyOrInsertPosition, true);
    }
