package com.google.android.gms.internal.measurement;

import java.util.List;
import java.util.Map;

/* JADX INFO: Access modifiers changed from: package-private */
/* loaded from: classes.dex */
public interface zzhe {
    int zza();

    <T> T zza(zzhd<T> zzhdVar, zzeq zzeqVar);

    void zza(List<Double> list);

    <T> void zza(List<T> list, zzhd<T> zzhdVar, zzeq zzeqVar);

    <K, V> void zza(Map<K, V> map, zzgf<K, V> zzgfVar, zzeq zzeqVar);

    int zzb();

    @Deprecated
    <T> T zzb(zzhd<T> zzhdVar, zzeq zzeqVar);

    void zzb(List<Float> list);

    @Deprecated
    <T> void zzb(List<T> list, zzhd<T> zzhdVar, zzeq zzeqVar);

    void zzc(List<Long> list);

    boolean zzc();

    double zzd();

    void zzd(List<Long> list);

    float zze();

    void zze(List<Integer> list);

    long zzf();

    void zzf(List<Long> list);

    long zzg();

    void zzg(List<Integer> list);

    int zzh();

    void zzh(List<Boolean> list);

    long zzi();

    void zzi(List<String> list);

    int zzj();

    void zzj(List<String> list);

    void zzk(List<zzdu> list);

    boolean zzk();

    String zzl();

    void zzl(List<Integer> list);

    String zzm();

    void zzm(List<Integer> list);

    zzdu zzn();

    void zzn(List<Integer> list);

    int zzo();

    void zzo(List<Long> list);

    int zzp();

    void zzp(List<Integer> list);

    int zzq();

    void zzq(List<Long> list);

    long zzr();

    int zzs();

    long zzt();
}
