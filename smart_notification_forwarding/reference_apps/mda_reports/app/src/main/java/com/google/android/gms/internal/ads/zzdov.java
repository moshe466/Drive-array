...
public final class zzdov {
    ...
    public static KeyPair zza(ECParameterSpec eCParameterSpec) {
        KeyPairGenerator zzhd = zzdoy.zzhgl.zzhd("EC");
        zzhd.initialize(eCParameterSpec);
        return zzhd.generateKeyPair();
    }

    public static ECPublicKey zza(zzdox zzdoxVar, byte[] bArr, byte[] bArr2) {
        ECParameterSpec zza = zza(zzdoxVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        a(eCPoint, zza.getCurve());
        return (ECPublicKey) zzdoy.zzhgm.zzhd("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static ECParameterSpec zza(zzdox zzdoxVar) {
        String str;
        String str2;
        String str3;
        String str4;
        String str5;
        int i = zzdou.b[zzdoxVar.ordinal()];
        if (i == 1) {
            ...
            if (i != 3) {
                String valueOf = String.valueOf(zzdoxVar);
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) {
        ...
        PublicKey generatePublic = zzdoy.zzhgm.zzhd("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhd = zzdoy.zzhgk.zzhd("ECDH");
        zzhd.init(eCPrivateKey);
        try {
            zzhd.doPhase(generatePublic, true);
            byte[] generateSecret = zzhd.generateSecret();
